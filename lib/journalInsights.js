...
function describeRatingPublishTarget({
  ...
  return {
    ...
    note: targetKind === 'course'
      ? (isEn
        ? 'The same published snapshot will be reused in Journal Rating, My Day, and message surfaces for this course.'
        : 'Той самий published snapshot зʼявиться в рейтингу журналу, My Day і message surfaces для цього курсу.')
      : (isEn
        ? 'Only this subject scope will receive the snapshot, and the same card stays consistent across Journal Rating, My Day, and messages.'
        : 'Лише цей subject scope отримає snapshot, а одна й та сама картка лишиться консистентною між рейтингом журналу, My Day і повідомленнями.'),
  };
}
...
function buildAttendanceHealthSummary({
  ...
  const hasExplicitTotal = total !== null && typeof total !== 'undefined' && Number.isFinite(Number(total));
  const markedTotal = hasExplicitTotal
    ? Number(total)
  ...
  const hasExplicitRecentTotal = recentTotal !== null && typeof recentTotal !== 'undefined' && Number.isFinite(Number(recentTotal));
  const normalizedRecentTotal = hasExplicitRecentTotal
    ? Number(recentTotal)
