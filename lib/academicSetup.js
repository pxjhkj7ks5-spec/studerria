...
function resolveAdminAcademicScopeState({
  ...
  const fallbackCourse = requestedCourseId
    ...
    : null;
  const requestedCourseContext = requestedCourseId
    ? (normalizedStudyContexts.find((context) => Number(context.course_id || 0) === Number(requestedCourseId || 0)) || null)
    : null;
  const availableCourseIds = hasScopedCourseFilter
    ? Array.from(normalizedAllowedCourseIds.values())
    : normalizedCourses
      .map((course) => Number(course.id || 0))
      .filter((value) => Number.isInteger(value) && value > 0);
  if (preferCourseOnly && requestedCourseId && fallbackCourse) {
    const inferredTrack = typeof inferTrackFromCourse === 'function'
      ? normalizeSelectableTrack(inferTrackFromCourse(fallbackCourse))
      : '';
    const selectedCampusLabel = normalizeCampusKey(fallbackCourse.location || '', 'kyiv') === 'munich'
      ? 'Munich'
      : 'Kyiv';
    return {
      track: '',
      ...
      label: String(fallbackCourse.name || 'Academic scope'),
      selectedCourse: fallbackCourse,
      selectedContext: null,
      selectedTrackLabel: inferredTrack === 'master'
        ? 'Masters'
        : (inferredTrack === 'teacher' ? 'Teachers' : (inferredTrack === 'bachelor' ? 'Bachelors' : '')),
  ...
  if (!explicitContext && requestedCourseId && fallbackCourse && !requestedCourseContext) {
    const inferredTrack = typeof inferTrackFromCourse === 'function'
      ? normalizeSelectableTrack(inferTrackFromCourse(fallbackCourse))
      : '';
    const selectedCampusLabel = normalizeCampusKey(fallbackCourse.location || '', 'kyiv') === 'munich'
...
async function listLegacyCourseHomeworkRows(store, {
  homeworkIds = [],
  courseId,
  selectClause = 'h.*',
  extraWhere = '',
  ...
  const normalizedCourseId = normalizePositiveInt(courseId);
  const normalizedHomeworkIds = Array.from(new Set(
    (Array.isArray(homeworkIds) ? homeworkIds : [homeworkIds])
      .map((value) => normalizePositiveInt(value))
      .filter((value) => Number.isInteger(value) && value > 0)
  ));
  if (!normalizedCourseId || !normalizedHomeworkIds.length || !store || typeof store.all !== 'function') {
    return [];
  }
  const safeSelectClause = String(selectClause || 'h.*').trim() || 'h.*';
  const safeExtraWhere = typeof extraWhere === 'string' ? extraWhere.trim() : '';
  const normalizedExtraParams = Array.isArray(extraParams) ? extraParams : [];
  const placeholders = normalizedHomeworkIds.map(() => '?').join(',');
  try {
    const rows = await store.all(
      `
        SELECT DISTINCT ${safeSelectClause}
        FROM homework h
        JOIN subject_course_bindings scb ON scb.subject_id = h.subject_id
        WHERE h.id IN (${placeholders})
          AND scb.course_id = ?
          ${safeExtraWhere ? `AND ${safeExtraWhere}` : ''}
      `,
      [...normalizedHomeworkIds, normalizedCourseId, ...normalizedExtraParams]
    );
    return Array.isArray(rows) ? rows : [];
  } catch (err) {
    if (!isLegacySchemaCompatibilityError(err)) {
...
async function listLegacyCourseStudentGroupAssignments(store, {
  courseId,
  studentIds = [],
  subjectIds = [],
  ...
  const normalizedStudentIds = Array.from(new Set(
  ...
  const where = [];
  const params = [];
  const subjectNameSelect = includeSubjectNames === false
  ...
  if (normalizedStudentIds.length) {
    where.push('sg.student_id = ANY(?::int[])');
    params.push(normalizedStudentIds);
  }
  if (normalizedSubjectIds.length) {
    where.push('sg.subject_id = ANY(?::int[])');
  ...
  try {
    const rows = await store.all(
      `
        ...
        JOIN subject_course_bindings scb ON scb.subject_id = s.id
        WHERE ${where.join('\n          AND ')}
        ORDER BY sg.student_id ASC, sg.subject_id ASC, sg.group_number ASC
      `,
      params
    );
    return Array.isArray(rows) ? rows : [];
    ...
    `);
    fallbackParams.push(normalizedCourseId, normalizedCourseId);
    if (normalizedStudentIds.length) {
      fallbackWhere.push('sg.student_id = ANY(?::int[])');
      fallbackParams.push(normalizedStudentIds);
    }
    if (normalizedSubjectIds.length) {
      fallbackWhere.push('sg.subject_id = ANY(?::int[])');
      fallbackParams.push(normalizedSubjectIds);
    }
    if (activeOnly === true) {
      fallbackWhere.push(normalizeLegacyActiveFlagSql('u.is_active', '1'));
    }
    const rows = await store.all(
      `
        SELECT
...
async function countLegacySubjectStudents(store, {
  courseId,
  subjectIds = [],
  groupNumber = null,
  activeOnly = true,
  ...
  const normalizedGroupNumber = normalizePositiveInt(groupNumber);
  if (!normalizedCourseId || !normalizedSubjectIds.length || !store || typeof store.all !== 'function') {
    return [];
  }
  const where = [
    'scb.course_id = ?',
    'sg.subject_id = ANY(?::int[])',
    `(
  ...
  const params = [normalizedCourseId, normalizedSubjectIds, normalizedCourseId, normalizedCourseId];
  if (normalizedGroupNumber) {
    where.push('sg.group_number = ?');
  ...
  try {
    const rows = await store.all(
      `
        SELECT
          sg.subject_id,
          COUNT(DISTINCT sg.student_id)::int AS students_total
        FROM student_groups sg
        JOIN users u ON u.id = sg.student_id
    ...
    const fallbackParams = [normalizedCourseId, normalizedSubjectIds, normalizedCourseId, normalizedCourseId];
    if (normalizedGroupNumber) {
      fallbackWhere.push('sg.group_number = ?');
      fallbackParams.push(normalizedGroupNumber);
    }
    if (activeOnly === true) {
      fallbackWhere.push(normalizeLegacyActiveFlagSql('u.is_active', '1'));
    }
...
async function countLegacyMessageRecipients(store, {
  courseId,
  targets = [],
  activeOnly = true,
} = {}) {
  ...
  const normalizedTargets = Array.from(new Map(
    (Array.isArray(targets) ? targets : [targets])
      .map((target) => {
        const subjectId = normalizePositiveInt(target && target.subject_id);
        const groupNumber = normalizePositiveInt(target && target.group_number);
        if (!subjectId || !groupNumber) {
          return null;
        }
        return [`${subjectId}|${groupNumber}`, { subject_id: subjectId, group_number: groupNumber }];
      })
      .filter(Boolean)
  ).values());
  if (!normalizedCourseId || !normalizedTargets.length || !store || typeof store.all !== 'function') {
    return [];
  }
  const valuesSql = normalizedTargets.map(() => '(?, ?)').join(', ');
  const params = [];
  normalizedTargets.forEach((target) => {
    params.push(target.subject_id, target.group_number);
  });
  params.push(normalizedCourseId, normalizedCourseId, normalizedCourseId);
  const rows = await store.all(
    `
      WITH target_scope (subject_id, group_number) AS (
        VALUES ${valuesSql}
      )
      SELECT
        ts.subject_id,
        ts.group_number,
        COUNT(DISTINCT u.id)::int AS students_total
      FROM target_scope ts
      LEFT JOIN student_groups sg
        ON sg.subject_id = ts.subject_id
       AND sg.group_number = ts.group_number
      LEFT JOIN users u
        ON u.id = sg.student_id
      LEFT JOIN academic_v2_groups v2_group
        ON v2_group.id = u.group_id
      WHERE (
        COALESCE(v2_group.legacy_course_id, 0) = ?
        OR (
          u.group_id IS NULL
        ...
        AND EXISTS (
          SELECT 1
          FROM subject_course_bindings scb
          WHERE scb.subject_id = ts.subject_id
            AND scb.course_id = ?
        )
       ${activeOnly === true ? `AND ${normalizeLegacyActiveFlagSql('u.is_active', '1')}` : ''}
      GROUP BY ts.subject_id, ts.group_number
      ORDER BY ts.subject_id ASC, ts.group_number ASC
    `,
    params
  );
  return Array.isArray(rows) ? rows : [];
...
async function listLegacyTeacherHomeworkRows(store, {
  userId,
  courseIds = [],
  subjectIds = [],
  semesterIds = [],
  ...
  const where = [
    'h.created_by_id = ?',
    "COALESCE(h.is_teacher_homework, 0) = 1",
  ];
  const params = [normalizedUserId];
  if (normalizedSubjectIds.length) {
    where.push('h.subject_id = ANY(?::int[])');
    params.push(normalizedSubjectIds);
  }
  if (normalizedCourseIds.length) {
    where.push('h.course_id = ANY(?::int[])');
    params.push(normalizedCourseIds);
  }
  if (normalizedSemesterIds.length) {
    where.push('(h.semester_id = ANY(?::int[]) OR h.semester_id IS NULL)');
    params.push(normalizedSemesterIds);
  }
  const rows = await store.all(
    `
      SELECT
...
async function getLegacyCourseUser(store, {
  ...
  const params = [normalizedUserId, normalizedCourseId, normalizedCourseId];
  const row = await store.get(
    `
      SELECT u.*
      FROM users u
...
const LEGACY_COURSE_PRIMARY_BLOCKER_KEYS = new Set(['users', 'subjects', 'semesters']);

const LEGACY_COURSE_BLOCKER_LABELS = Object.freeze({
  users: 'Users',
  subjects: 'Subjects',
  semesters: 'Semesters',
...
function formatLegacyCourseBlockerLabel(tableName, columnName = 'course_id') {
  const normalizedTableName = String(tableName || '').trim().toLowerCase();
  const normalizedColumnName = String(columnName || '').trim().toLowerCase();
  const key = normalizedColumnName && normalizedColumnName !== 'course_id'
    ? `${normalizedTableName}.${normalizedColumnName}`
    : normalizedTableName;
  if (Object.prototype.hasOwnProperty.call(LEGACY_COURSE_BLOCKER_LABELS, key)) {
    return LEGACY_COURSE_BLOCKER_LABELS[key];
  }
  const source = normalizedColumnName && normalizedColumnName !== 'course_id'
    ? `${normalizedTableName} ${normalizedColumnName}`
    : normalizedTableName;
  return source
    .split(/[_\s]+/)
...
async function listLegacyCourseBlockingReferences(store) {
  ...
  try {
    const rows = await store.all(
      `
        ...
        JOIN pg_class target_class ON target_class.oid = constraint_meta.confrelid
        JOIN pg_namespace target_ns ON target_ns.oid = target_class.relnamespace
        JOIN pg_class source_class ON source_class.oid = constraint_meta.conrelid
        JOIN pg_namespace source_ns ON source_ns.oid = source_class.relnamespace
        JOIN LATERAL unnest(constraint_meta.conkey) WITH ORDINALITY AS source_keys(attnum, ordinality) ON TRUE
        JOIN LATERAL unnest(constraint_meta.confkey) WITH ORDINALITY AS target_keys(attnum, ordinality)
          ON target_keys.ordinality = source_keys.ordinality
        JOIN pg_attribute source_attr ON source_attr.attrelid = source_class.oid AND source_attr.attnum = source_keys.attnum
        JOIN pg_attribute target_attr ON target_attr.attrelid = target_class.oid AND target_attr.attnum = target_keys.attnum
        WHERE constraint_meta.contype = 'f'
          AND target_ns.nspname = current_schema()
          AND source_ns.nspname = current_schema()
          AND target_class.relname = 'courses'
          AND target_attr.attname = 'id'
          AND constraint_meta.confdeltype IN ('a', 'r')
          AND source_attr.attnum > 0
          AND NOT source_attr.attisdropped
        ORDER BY source_ns.nspname ASC, source_class.relname ASC, source_attr.attname ASC
      `
    );
    return Array.isArray(rows) ? rows : [];
  } catch (_) {
...
async function getLegacyCourseDependencyCounts(store, courseId) {
  ...
  const [userRow, subjectRow, semesterRow, blockerReferences] = await Promise.all([
    ...
    listLegacyCourseBlockingReferences(store),
  ]);
  const usersCount = Number(userRow && (userRow.count ?? userRow.cnt) || 0);
  const subjectsCount = Number(subjectRow && (subjectRow.count ?? subjectRow.cnt) || 0);
  const semestersCount = Number(semesterRow && (semesterRow.count ?? semesterRow.cnt) || 0);
  const blockerRows = new Map([
    ['users', {
      key: 'users',
      label: formatLegacyCourseBlockerLabel('users'),
      table_name: 'users',
    ...
    ['subjects', {
      key: 'subjects',
      label: formatLegacyCourseBlockerLabel('subjects'),
      table_name: 'subjects',
      column_name: 'course_id',
      count: subjectsCount,
      primary: true,
    ...
    ['semesters', {
      key: 'semesters',
      label: formatLegacyCourseBlockerLabel('semesters'),
  ...
  await Promise.all(
    (Array.isArray(blockerReferences) ? blockerReferences : []).map(async (reference) => {
      const schemaName = String(reference && reference.schema_name ? reference.schema_name : '').trim();
      const tableName = String(reference && reference.table_name ? reference.table_name : '').trim();
      const columnName = String(reference && reference.column_name ? reference.column_name : '').trim();
      if (!schemaName || !tableName || !columnName) {
        return;
      }
      const key = columnName === 'course_id' ? tableName : `${tableName}.${columnName}`;
      if (blockerRows.has(key)) {
        return;
      }
      const countRow = await store.get(
        `
          SELECT COUNT(*) AS count
          FROM ${quotePgIdentifier(schemaName)}.${quotePgIdentifier(tableName)}
          WHERE ${quotePgIdentifier(columnName)} = ?
        `,
        [normalizedCourseId]
      );
      blockerRows.set(key, {
        key,
        label: formatLegacyCourseBlockerLabel(tableName, columnName),
        table_name: tableName,
        column_name: columnName,
        count: Number(countRow && (countRow.count ?? countRow.cnt) || 0),
        primary: LEGACY_COURSE_PRIMARY_BLOCKER_KEYS.has(key),
      });
    })
  );

  const orderedBlockerRows = Array.from(blockerRows.values())
    .sort((left, right) => {
  ...
  const blockingRows = orderedBlockerRows.filter((row) => Number(row.count || 0) > 0);
  const otherBlockingRows = blockingRows.filter((row) => !row.primary);
  const blockingSummary = orderedBlockerRows.reduce((accumulator, row) => {
    accumulator[row.key] = Number(row.count || 0);
    return accumulator;
  }, {});
  ...
  return {
    ...
    blocking_total: blockingRows.reduce((sum, row) => sum + Number(row.count || 0), 0),
    primary_blocking_total: blockingRows
      .filter((row) => row.primary)
      .reduce((sum, row) => sum + Number(row.count || 0), 0),
    other_blocking_total: otherBlockingRows.reduce((sum, row) => sum + Number(row.count || 0), 0),
    blocker_rows: orderedBlockerRows,
    blocking_rows: blockingRows,
    other_blocking_rows: otherBlockingRows,
    blocking_summary: blockingSummary,
    deletable: blockingRows.length === 0,
  };
}
