...
async function listLegacyCourseStudentGroupAssignments(store, {
  ...
  try {
    ...
    fallbackWhere.push('s.course_id = ?');
    fallbackParams.push(normalizedCourseId);
    if (normalizedStudentIds.length) {
...
async function countLegacySubjectStudents(store, {
  ...
  const where = ['scb.course_id = ?', 'sg.subject_id = ANY(?::int[])', 'u.course_id = ?'];
  const params = [normalizedCourseId, normalizedSubjectIds, normalizedCourseId];
  ...
  try {
    ...
    const fallbackWhere = ['s.course_id = ?', 'sg.subject_id = ANY(?::int[])', 'u.course_id = ?'];
    const fallbackParams = [normalizedCourseId, normalizedSubjectIds, normalizedCourseId];
    if (normalizedGroupNumber) {
