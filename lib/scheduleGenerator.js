...
function getSlotSequence(day, classNum) {
  const dayName = normalizeDay(day);
  const dayIndex = fullWeekDays.indexOf(dayName);
  const normalizedIndex = dayIndex >= 0 ? dayIndex : 0;
  const slot = clampNumber(classNum, 0);
  return normalizedIndex * 100 + slot;
}
...
function isGeneralSubject(item) {
  if (!item) return true;
  ...
  // Legacy rows may omit the flag; default behavior is "general subject".
  return true;
}

function shouldBlockParallelSubjectSlot(item) {
  if (!item) return false;
  if (!isSeminarLesson(item.lesson_type)) return false;
  return isGeneralSubject(item);
}

function shouldPreferParallelSeminarSlot(item) {
  if (!item) return false;
  if (!isSeminarLesson(item.lesson_type)) return false;
  return !shouldBlockParallelSubjectSlot(item);
}
...
function generateSchedule({
  ...
  const dayLoadScoreWeight = evennessWeight * 1.75;
  const courseDayScoreWeight = courseWeight * 1.6;
  const parallelSeminarRewardWeight = 0.9;
  const subjectSingleDay = config.subject_single_day !== false;
  const lectureSeminarSameDay = config.lecture_seminar_same_day !== false;
  const lectureBeforeSeminar = config.lecture_before_seminar !== false;
  ...
  const strictNoEvening = Boolean(config.strict_no_evening);
  ...
  const expandedItems = items.flatMap((item) => {
    ...
    if (!isSeminarLesson(item.lesson_type)) return [item];
  ...
  expandedItems.forEach((item) => {
    ...
    if (isSeminarLesson(item.lesson_type)) meta.hasSeminar = true;
  ...
  const lectureFirstSubjectKeys = new Set();
  if (lectureBeforeSeminar) {
    subjectMeta.forEach((meta, key) => {
      if (meta.hasLecture && meta.hasSeminar) {
        lectureFirstSubjectKeys.add(key);
      }
    });
  }
  ...
  const courseTotalLoad = new Map();
  const subjectParallelSlot = new Map();
  const subjectLectureSlot = new Map();
  ...
  const getLecturePriority = (item) => {
    if (!lectureBeforeSeminar) return 1;
    const subjectKey = getSubjectDayKey(item);
    if (!subjectKey || !lectureFirstSubjectKeys.has(subjectKey)) return 1;
    return isLectureLesson(item.lesson_type) ? 0 : 1;
  };
  const sortedItems = [...mirrorPairsResult.remaining].sort((a, b) => {
    ...
    const lecturePriorityA = getLecturePriority(a);
    const lecturePriorityB = getLecturePriority(b);
    if (lecturePriorityA !== lecturePriorityB) return lecturePriorityA - lecturePriorityB;
  ...
  const incCourseDayLoad = (courseId, day, count) => {
    ...
    const totalKey = String(courseId);
    const totalNext = (courseTotalLoad.get(totalKey) || 0) + count;
    courseTotalLoad.set(totalKey, totalNext);
  };
  const getCourseDayLoad = (courseId, day) => courseDayLoad.get(`${courseId}|${day}`) || 0;
  const getCourseAverageDayLoad = (courseId) => {
    const total = courseTotalLoad.get(String(courseId)) || 0;
    const context = courseContexts.get(String(courseId));
    const dayCount = context && Array.isArray(context.active_days) && context.active_days.length
      ? context.active_days.length
      : 5;
    return total / Math.max(1, dayCount);
  };
  ...
  const calcLateSlotPenalty = (classNum) => {
    ...
    if (!late) return 0;
    const weight = 0.8 + lateSlotPenaltyScale * 4.5;
    return late * late * weight + late * lateSlotPenaltyScale * 1.2;
  };
  ...
  const calcDayLoadPenalty = (courseId, groups, day, weeksCount) => {
    ...
    return maxLoad * 1.05 + overTarget * 2.2;
  };
  ...
  const calcCourseDayPenalty = (courseId, day, weeksCount) => {
    ...
    const perGroupLoad = load / Math.max(1, groupCount);
    const overTarget = Math.max(0, perGroupLoad - targetDailyPairs);
    const averagePerGroupLoad = (getCourseAverageDayLoad(courseId) / Math.max(1, weeksCount)) / Math.max(1, groupCount);
    const skew = Math.max(0, perGroupLoad - averagePerGroupLoad);
    return perGroupLoad * 0.95 + overTarget * 2.6 + skew * 2.4;
  };

  const markSubjectParallelSlot = (subjectId, week, day, classNum) => {
    if (!subjectId) return;
    const key = `${subjectId}|${week}|${day}|${classNum}`;
    subjectParallelSlot.set(key, (subjectParallelSlot.get(key) || 0) + 1);
  };
  const getSubjectParallelSlotLoad = (subjectId, week, day, classNum) => {
    if (!subjectId) return 0;
    return subjectParallelSlot.get(`${subjectId}|${week}|${day}|${classNum}`) || 0;
  };
  const calcParallelSeminarReward = (item, weeks, day, classNum) => {
    if (!shouldPreferParallelSeminarSlot(item)) return 0;
    if (!Array.isArray(weeks) || !weeks.length) return 0;
    let hits = 0;
    weeks.forEach((week) => {
      if (getSubjectParallelSlotLoad(item.subject_id, week, day, classNum) > 0) {
        hits += 1;
      }
    });
    return hits;
  ...
  const getSubjectWeekKey = (item, week) => {
    const subjectKey = getSubjectDayKey(item);
    if (!subjectKey) return '';
    return `${subjectKey}|${week}`;
  };
  const hasLectureFirstRule = (item) => {
    if (!lectureBeforeSeminar) return false;
    const subjectKey = getSubjectDayKey(item);
    if (!subjectKey) return false;
    return lectureFirstSubjectKeys.has(subjectKey);
  };
  const markSubjectLectureSlot = (item, week, day, classNum) => {
    if (!isLectureLesson(item.lesson_type)) return;
    const key = getSubjectWeekKey(item, week);
    if (!key) return;
    const sequence = getSlotSequence(day, classNum);
    const current = subjectLectureSlot.get(key);
    if (!current || sequence < current.sequence) {
      subjectLectureSlot.set(key, { day, classNum, sequence });
    }
  };
  const getSubjectLectureSlot = (item, week) => {
    const key = getSubjectWeekKey(item, week);
    if (!key) return null;
    return subjectLectureSlot.get(key) || null;
  };
  const violatesLectureFirstRule = (item, week, day, classNum) => {
    if (!isSeminarLesson(item.lesson_type)) return false;
    if (!hasLectureFirstRule(item)) return false;
    const lectureSlot = getSubjectLectureSlot(item, week);
    if (!lectureSlot) return false;
    const seminarSequence = getSlotSequence(day, classNum);
    return seminarSequence <= lectureSlot.sequence;
  };
  ...
  const scheduleMirrorPair = (pair) => {
    ...
    const candidateClasses = Array.from({ length: maxClass }, (_, idx) => idx + 1)
      .filter((classNum) => !strictNoEvening || classNum <= 5);
    if (!candidateClasses.length) {
      conflicts.push({
        item_id: `${itemA.id}|${itemB.id}`,
        subject: `${itemA.subject_name} ↔ ${itemB.subject_name}`,
        reason: 'no_allowed_classes',
      });
      return;
    }
    ...
    candidateDays.forEach((day) => {
      candidateClasses.forEach((classNum) => {
        ...
        const score = dayPenalty * dayLoadScoreWeight
          + coursePenalty * courseDayScoreWeight
          + slotPenalty * compactWeight
          + classPenalty
          + latePenalty
          + jitter;
    ...
    slotCandidates.forEach((slot) => {
      ...
      slotAttempts.push({
        slot,
        weeksA: attempt.weeksA,
        weeksB: attempt.weeksB,
        total,
        minSide: Math.min(attempt.weeksA.length, attempt.weeksB.length),
      });
    });
    if (slotAttempts.length) {
      const maxTotal = slotAttempts.reduce((max, row) => Math.max(max, row.total), 0);
      const coverageFloor = Math.max(1, maxTotal - 1);
      const bestAttempt = slotAttempts
        .filter((row) => row.total >= coverageFloor)
        .sort((a, b) =>
          a.slot.score - b.slot.score
          || b.minSide - a.minSide
          || b.total - a.total
        )[0];
      if (bestAttempt) {
        bestSlot = bestAttempt.slot;
        bestWeeksA = bestAttempt.weeksA;
        bestWeeksB = bestAttempt.weeksB;
      }
    }
    ...
    const applyEntry = (item, groupNum, week) => {
      ...
      if (shouldPreferParallelSeminarSlot(item)) {
        markSubjectParallelSlot(item.subject_id, week, bestSlot.day, bestSlot.classNum);
      }
      ...
      if (lecture) {
        ...
        markSubjectLectureSlot(item, week, bestSlot.day, bestSlot.classNum);
      }
  ...
  const mirrorPairsEarly = [];
  const mirrorPairsAfterLectures = [];
  mirrorPairs.forEach((pair) => {
    if (hasLectureFirstRule(pair.a) || hasLectureFirstRule(pair.b)) {
      mirrorPairsAfterLectures.push(pair);
    } else {
      mirrorPairsEarly.push(pair);
    }
  });

  mirrorPairsEarly.forEach((pair) => scheduleMirrorPair(pair));
  ...
  for (const item of sortedItems) {
    ...
    const canUseWeek = (week, day, classNum) => {
      ...
      if (shouldBlockParallelSubjectSlot(item) && hasSubjectSlot(item.subject_id, week, day, classNum)) {
        return false;
      }
      if (violatesLectureFirstRule(item, week, day, classNum)) {
        return false;
      }
    ...
    const explainWeekRejection = (week, day, classNum) => {
      const lecture = isLectureLesson(item.lesson_type);
      if (lecture) {
        if (hasCourseSlot(item.course_id, week, day, classNum)) return 'course_slot_busy';
      } else if (hasCourseLectureSlot(item.course_id, week, day, classNum)) {
        return 'lecture_slot_busy';
      }
      if (shouldBlockParallelSubjectSlot(item) && hasSubjectSlot(item.subject_id, week, day, classNum)) {
        return 'subject_slot_busy';
      }
      if (violatesLectureFirstRule(item, week, day, classNum)) {
        return 'lecture_must_be_first';
      }
      if (item.teacher_id && hasTeacherSlot(item.teacher_id, week, day, classNum)) {
        return 'teacher_slot_busy';
      }
      for (const groupNum of groupList) {
        if (hasGroupSlot(item.course_id, groupNum, week, day, classNum)) {
          return 'group_slot_busy';
        }
        if (getGroupDaily(item.course_id, groupNum, week, day) >= maxDailyPairs) {
    ...
    let slotPasses = 0;
    const maxSlotPasses = Math.max(1, Math.min(targetPairs, Math.max(1, candidateClasses.length * 2)));
    let localLockedDay = lockedDay || null;

    const applyPlan = (slot, weeks) => {
      weeks.forEach((week) => {
        const lecture = isLectureLesson(item.lesson_type);
        if (shouldBlockParallelSubjectSlot(item)) {
          markSubjectSlot(item.subject_id, week, slot.day, slot.classNum);
        }
        if (shouldPreferParallelSeminarSlot(item)) {
          markSubjectParallelSlot(item.subject_id, week, slot.day, slot.classNum);
        }
        lockSubjectDay(item, slot.day);
        markCourseSlot(item.course_id, week, slot.day, slot.classNum);
        if (lecture) {
          markCourseLectureSlot(item.course_id, week, slot.day, slot.classNum);
          markSubjectLectureSlot(item, week, slot.day, slot.classNum);
        }
        markTeacherSlot(item.teacher_id, week, slot.day, slot.classNum);
        incTeacherWeekly(item.teacher_id, week);
        for (const groupNum of groupList) {
          markGroupSlot(item.course_id, groupNum, week, slot.day, slot.classNum);
          incGroupDaily(item.course_id, groupNum, week, slot.day);
          incGroupDayLoad(item.course_id, groupNum, slot.day, 1);
          incCourseDayLoad(item.course_id, slot.day, 1);
          addGroupDaySlot(item.course_id, groupNum, slot.day, slot.classNum);
          results.push({
            item_id: item.id,
            course_id: item.course_id,
            semester_id: item.semester_id,
      ...
      scheduledPairs += weeks.length;
      if (!localLockedDay && shouldLockSubjectDay(item)) {
        localLockedDay = slot.day;
      }
    };

    while (scheduledPairs < targetPairs && slotPasses < maxSlotPasses) {
      const needed = targetPairs - scheduledPairs;
      const slotAttempts = [];
      const slotCandidates = buildSlotCandidates();
      for (const slot of slotCandidates) {
        if (localLockedDay && slot.day !== localLockedDay) continue;
        const tryWeeks = pickWeeks(availableWeeks, needed, distribution, (week) =>
          canUseWeek(week, slot.day, slot.classNum)
        );
        if (!tryWeeks.length) continue;
        const parallelReward = calcParallelSeminarReward(item, tryWeeks, slot.day, slot.classNum);
        const adjustedScore = slot.score - parallelReward * parallelSeminarRewardWeight;
        slotAttempts.push({
          slot,
          weeks: tryWeeks,
          parallelReward,
      ...
      if (!slotAttempts.length) break;
      const maxCoverage = slotAttempts.reduce((max, row) => Math.max(max, row.weeks.length), 0);
      const coverageFloor = Math.max(1, maxCoverage - 1);
      const bestAttempt = slotAttempts
        .filter((row) => row.weeks.length >= coverageFloor)
        .sort((a, b) =>
          b.parallelReward - a.parallelReward
          || a.adjustedScore - b.adjustedScore
          || b.weeks.length - a.weeks.length
        )[0];
      if (!bestAttempt || !bestAttempt.weeks.length) break;
      applyPlan(bestAttempt.slot, bestAttempt.weeks);
      slotPasses += 1;
    }
    ...
    if (!scheduledPairs) {
      ...
      for (const slot of slotCandidates) {
        ...
        for (const week of availableWeeks) {
          const reason = explainWeekRejection(week, slot.day, slot.classNum) || 'unknown';
          rejectionMap.set(reason, (rejectionMap.get(reason) || 0) + 1);
        }
      }
      const topRejections = Array.from(rejectionMap.entries())
      ...
      conflicts.push({
        ...
        diagnostics: {
          ...
          available_weeks: availableWeeks.length,
    ...
    if (scheduledPairs < targetPairs) {
      conflicts.push({
        item_id: item.id,
        subject: item.subject_name,
        reason: 'partial_schedule',
        scheduled: scheduledPairs,
        target: targetPairs,
      });
    }
  }

  mirrorPairsAfterLectures.forEach((pair) => scheduleMirrorPair(pair));
