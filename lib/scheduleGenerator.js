...
function buildMirrorPairs(items, enabled) {
  ...
  buckets.forEach((bucket) => {
    bucket.group1.sort((a, b) => Number(a.id) - Number(b.id));
    bucket.group2.sort((a, b) => Number(a.id) - Number(b.id));
...
function generateSchedule({
  ...
  if (autoSubjectDays) {
    ...
    subjectMeta.forEach((meta, key) => {
      ...
      candidateDays.forEach((day, idx) => {
        const loadKey = `${meta.courseId}|${day}`;
        const load = subjectDayLoad.get(loadKey) || 0;
        const score = load * 2 + idx * 0.1;
        if (score < bestScore) {
          bestScore = score;
          chosen = day;
        }
  ...
  const calcLateSlotPenalty = (classNum) => {
    ...
    if (!late) return 0;
    return late * lateSlotPenaltyScale;
  };
  ...
  const scheduleMirrorPair = (pair) => {
    ...
    candidateDays.forEach((day) => {
      candidateClasses.forEach((classNum) => {
        ...
        const score = dayPenalty * evennessWeight + coursePenalty * courseWeight + slotPenalty * compactWeight + classPenalty + latePenalty;
