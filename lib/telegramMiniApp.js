...
function buildTelegramDisplayName(user = {}) {
  ...
  return [normalizedUser.first_name, normalizedUser.last_name]
...
function normalizeTelegramIdentityCandidate(value = '') {
  return String(value || '')
    .trim()
    .replace(/\s+/g, ' ')
    .toLowerCase();
...
function buildTelegramRecoveryIdentityCandidates(user = {}) {
  const normalizedUser = normalizeTelegramUser(user || {});
  const displayName = buildTelegramDisplayName(normalizedUser);
  const username = normalizedUser.username ? normalizedUser.username.replace(/^@/, '') : '';
  return Array.from(new Set([
    displayName,
    username,
    username ? `@${username}` : '',
    normalizedUser.id ? `Telegram ${normalizedUser.id}` : '',
  ]
    .map(normalizeTelegramIdentityCandidate)
    .filter(Boolean)));
}
