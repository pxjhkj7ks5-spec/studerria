...
const DAY_LABELS = ['Неділя', 'Понеділок', 'Вівторок', 'Середа', 'Четвер', 'Пʼятниця', 'Субота'];
...
function getDailyScheduleConfig(env = process.env) {
  ...
  return {
    ...
    checkIntervalMs: Number.isFinite(interval) ? Math.max(10000, Math.min(300000, Math.floor(interval))) : 30000,
  };
}
...
function formatBlock(block, compact) {
  ...
  // Code entities also prevent Telegram from auto-linking unusual names.
  const names = shown.map((user) => `<code>${escapeHtml(studentName(user))}</code>`).join(', ');
  const hidden = block.students.length - shown.length;
  const namesLine = `${names}${hidden ? `, ще ${hidden}` : ''}`;
  const labels = {
    empty: '😎 Завтра чілім — пар немає',
    incomplete: '🛠 Розклад ще не налаштовано',
  ...
  const lines = [`👥 ${namesLine}`];
  for (const lesson of block.lessons) {
    lines.push(`${lesson.classNumber} пара${lesson.time ? ` · ${escapeHtml(lesson.time)}` : ''} · <b>${escapeHtml(lesson.subject)}</b>`);
    lines.push([TYPE_LABELS[lesson.type] || 'Пара',
...
function buildDailyScheduleText(digest) {
  ...
  const courseLabel = clean([digest.course.program_name, digest.course.admission_year, digest.course.label].filter(Boolean).join(' · '), 180);
  const header = `<b>🌙 Розклад на завтра</b>\n${DAY_LABELS[date.getUTCDay()]}, ${digest.targetIso.split('-').reverse().join('.')} · ${escapeHtml(courseLabel)}`;
  ...
  let text = render(digest.blocks, false);
...
function createDailyScheduleService(deps) {
  ...
  async function tick() {
    ...
    if (clock.hour * 60 + clock.minute < config.time.hour * 60 + config.time.minute) return;
    ...
    try {
      ...
      // A slow database must not publish yesterday's digest after midnight.
      if (getKyivClock(now()).dateIso !== clock.dateIso) return;
