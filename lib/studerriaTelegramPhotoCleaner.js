...
function getImageTypeFromExtension(extension = '') {
  ...
  if (normalized === 'jpg' || normalized === 'jpeg') return SUPPORTED_IMAGE_TYPES.get('image/jpeg');
...
function getStuderriaTelegramCleanerMedia(message = {}) {
  ...
  const filenameImageType = getImageTypeFromExtension(path.extname(document.file_name || ''));
  if (contentType && !contentType.startsWith('image/') && !filenameImageType) return null;
  if (!contentType && !filenameImageType) return null;
