...
function getStuderriaTelegramCleanerIngressContext(message = {}) {
  const document = message && message.document ? message.document : null;
  const text = String((message && (message.text || message.caption)) || '').trim();
  const photo = selectLargestTelegramPhotoSize(Array.isArray(message && message.photo) ? message.photo : []);
  const hasPhoto = Boolean(photo);
  const hasDocument = Boolean(document && document.file_id);
  const isCommand = text.startsWith('/');
  if (!hasPhoto && !hasDocument && !isCommand) return null;
  return {
    chatId: normalizeTelegramChatId(message && message.chat && message.chat.id),
    messageId: message && message.message_id ? message.message_id : null,
    mediaKind: hasPhoto ? 'photo' : (hasDocument ? 'document' : 'command'),
    fileSize: hasPhoto ? (Number(photo.file_size || 0) || null) : (Number(document && document.file_size || 0) || null),
    contentType: hasPhoto ? 'image/jpeg' : (document ? (normalizeImageMimeType(document.mime_type) || null) : null),
    fileExtension: hasPhoto ? 'jpg' : (document ? (path.extname(document.file_name || '').replace('.', '').toLowerCase() || null) : null),
...
function logStuderriaDevPhotoCleaner(logger, status, details = {}) {
  const payload = {
    ...
    has_text: typeof details.hasText === 'boolean' ? details.hasText : null,
  };
  if (status === 'ingress') {
    const log = logger && typeof logger.log === 'function' ? logger.log.bind(logger) : console.log;
    log('Studerria Telegram dev photo cleaner ingress', payload);
    return;
  }
  if (status === 'success') {
    const log = logger && typeof logger.log === 'function' ? logger.log.bind(logger) : console.log;
...
function logStuderriaTelegramDevPhotoCleanerIngress(message = {}, deps = {}) {
  const env = deps.env || process.env;
  if (!isStuderriaDevPhotoCleanerEnabled(env)) return false;
  const context = getStuderriaTelegramCleanerIngressContext(message);
  if (!context || !context.chatId || !getStuderriaDevPhotoCleanerChatIds(env).has(context.chatId)) return false;
  logStuderriaDevPhotoCleaner(deps.logger || console, 'ingress', context);
  return true;
}
