...
// Match known legacy spelling only; never guess a different catalog subject.
const SCHEDULE_SUBJECT_ALIASES = new Map([
  [
    'історія євроропейської цивілізації. культурна спадщина європи',
    'історія європейської цивілізації. культурна спадщина європи',
  ],
]);
...
function normalizeScheduleSubjectName(value) {
  const key = String(value || '')
    ...
    .replace(/[\s\u00a0]+/g, ' ')
    .trim();
  return SCHEDULE_SUBJECT_ALIASES.get(key) || key;
}

function indexScheduleSubjects(rows = [], nameFields = ['name']) {
  const index = new Map();
  for (const row of rows || []) {
    for (const field of nameFields) {
      const key = normalizeScheduleSubjectName(row[field]);
      if (!key) continue;
      if (!index.has(key)) index.set(key, []);
      const matches = index.get(key);
      if (!matches.some((item) => Number(item.id) === Number(row.id))) {
        matches.push(row);
      }
    }
  }
...
function preferScheduleSubjectScope(matches, termId) {
  if (matches.length < 2) return matches;
  const inTerm = matches.filter((row) => (row.term_ids || []).some((id) => Number(id) === Number(termId)));
  const scoped = inTerm.length ? inTerm : matches;
  const visible = scoped.filter((row) => row.is_visible === true || row.is_visible === 1);
  return visible.length ? visible : scoped;
}
