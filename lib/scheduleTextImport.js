...
const ACTIVITY_ALIASES = new Map([
  ['lecture', 'lecture'], ['лекція', 'lecture'], ['лекция', 'lecture'], ['лк', 'lecture'],
  ['seminar', 'seminar'], ['семінар', 'seminar'], ['семинар', 'seminar'], ['сем', 'seminar'],
...
function parseScheduleImportText(rawText, options = {}) {
  ...
  lines.forEach((rawLine, index) => {
    ...
    if (!line || line.startsWith('#') || /^STUDERRIA_SCHEDULE_V1$/i.test(line)) return;
    ...
    if (columns.length !== 6) {
      errors.push({ lineNumber, message: 'потрібно 6 колонок: предмет, тип, день, пара, тижні, групи' });
      return;
    }

    const [subject, rawActivity, rawDay, rawClassNumber, rawWeeks, rawGroups] = columns;
