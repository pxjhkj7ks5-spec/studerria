...
function getDemoScheduleRows(lang = 'uk', weekStart = startOfLocalWeek()) {
  ...
  return [
    ...
    class_date: formatLocalDate(addDays(weekStart, ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].indexOf(row.day_of_week))),
...
function buildScheduleLocals({ lang = 'uk', role = 'student', username = '', bellSchedule = {}, daysOfWeek = [] } = {}) {
  ...
  const rows = getDemoScheduleRows(lang, weekStart);
  ...
  activeDays.forEach((day) => {
    ...
    const weekdayIndex = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].indexOf(day);
    dayDates[day] = formatLocalDate(addDays(weekStart, Math.max(0, weekdayIndex)));
  });
