...
function canAccessOperationalDetails({
  ...
  if (normalizedStatusAccessToken && normalizedProvidedToken) {
    const tokenBuffer = Buffer.from(normalizedStatusAccessToken);
    const providedBuffer = Buffer.from(normalizedProvidedToken);
    if (tokenBuffer.length === providedBuffer.length && timingSafeEqual(tokenBuffer, providedBuffer)) {
      return true;
    }
  }
