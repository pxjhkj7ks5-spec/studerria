  ...
  return sourceRows
    .map((row) => {
      ...
      const stageTermNumbers = Array.isArray(stageRow.term_numbers)
        ...
        : [];
      const allowedByTerm = !normalizedTermNumber || !stageTermNumbers.length || stageTermNumbers.includes(normalizedTermNumber);
...
async function loadStudentSubjectCatalog(store, userOrId, options = {}) {
  ...
  if (scopeState.scope && String(scopeState.scope.track_key || '') === 'bachelor') {
