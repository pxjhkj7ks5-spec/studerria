...
async function loadUserRow(store, userOrId) {
  if (userOrId && typeof userOrId === 'object') {
    return {
      ...
      show_full_schedule: normalizeBoolean(userOrId.show_full_schedule, false),
    };
  }
...
function mapTermRow(row) {
  if (!row) return null;
  const weeksCount = normalizePositiveInt(row.weeks_count) || resolveTermWeeksCount(row.term_number);
  return {
    id: Number(row.id || 0),
    group_id: Number(row.group_id || 0),
...
function pickPreferredActiveTermRow(rows = []) {
...
async function getActiveGroupTerm(store, groupId) {
  ...
  return mapTermRow(pickPreferredActiveTermRow(rows));
}
...
async function loadStudentScheduleData(store, userOrId, options = {}) {
  ...
  const showFullSchedule = options.showFullSchedule === true
    || normalizeBoolean(subjectState.user && subjectState.user.show_full_schedule, false);
  ...
  const rows = await store.all(
    `
      SELECT
        ...
        gs.default_group,
        st.name AS template_name,
        room.label AS room_name,
        room.code AS room_code,
        COALESCE(NULLIF(TRIM(room.label), ''), NULLIF(TRIM(room.code), '')) AS room_label,
        COALESCE(day_format.delivery_mode, 'offline') AS delivery_mode
      ...
      LEFT JOIN academic_v2_group_day_formats day_format
        ON day_format.group_id = gs.group_id AND day_format.day_of_week = se.day_of_week
      WHERE gs.group_id = ?
        AND se.term_id = ?
        AND se.week_number = ?
  ...
  (rows || []).forEach((row) => {
    ...
    if (!isVisible) {
      ...
      included = true;
      reasonCode = 'included_full_schedule';
      compatHomeworkEnabled = Boolean(selectedSubject && legacySubjectId && compatCourseId && compatSemesterId);
      subject = visibleSubject || selectedSubject || {
        selected_group: null,
    ...
    } else if (visibleSubject && visibleSubject.opted_out) {
      reasonCode = 'dropped_subject_opted_out';
    ...
    const rowGroupNumber = showFullSchedule && activityType !== 'lecture'
      ? Math.max(1, Number(targetGroupNumbers[0] || row.group_number || 1))
      : activityType === 'lecture'
      ? (showFullSchedule ? effectiveDefaultGroup : selectedGroup)
      : Math.max(
        1,
        Number(
    ...
    const displayGroupNumbers = showFullSchedule && activityType !== 'lecture'
      ? targetGroupNumbers
      : activityType === 'lecture'
      ? (showFullSchedule ? [] : normalizeGroupNumberArray(
    ...
    scheduleRows.push({
      ...
      room_label: cleanText(row.room_label || row.room_name || row.room_code, 80),
      delivery_mode: ['offline', 'online', 'mixed'].includes(cleanText(row.delivery_mode, 20).toLowerCase())
        ? cleanText(row.delivery_mode, 20).toLowerCase()
        : 'offline',
      show_full_schedule: showFullSchedule,
    });
  });
