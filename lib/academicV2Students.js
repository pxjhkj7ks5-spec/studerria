...
async function listScopedGroupSubjectRows(store, groupId, termId = null) {
  ...
  const rows = await store.all(
    `
      ...
      WHERE gs.group_id = ?
        AND ${sqlTruthyExpr('gs.is_visible', true)}
        ${scopedTermSql}
...
async function loadStudentScheduleData(store, userOrId, options = {}) {
  ...
  (rows || []).forEach((row) => {
    ...
    const subject = selectedSubject || (
      ...
      && (
        normalizeBoolean(visibleSubject.is_required, false)
        || normalizeBoolean(visibleSubject.is_general, false)
        || Math.max(1, Number(visibleSubject.group_count || 0) || 1) === 1
      )
      && !normalizeBoolean(visibleSubject.opted_out, false)
        ? visibleSubject
        : null
    ) || (
      !legacySubjectId
    ...
    if (!legacySubjectId || !normalizePositiveInt(subjectState.scope.legacy_course_id)) {
    ...
    if (activityType !== 'lecture' && !targetGroupNumbers.includes(selectedGroup)) {
      return;
    }
    const rowGroupNumber = activityType === 'lecture'
      ? 1
      : Math.max(1, Number(targetGroupNumbers[0] || selectedGroup || subject.default_group || 1));

    scheduleRows.push({
      schedule_entry_id: Number(row.schedule_entry_id || 0),
      group_subject_id: Number(row.group_subject_id || 0),
      group_subject_activity_id: Number(row.group_subject_activity_id || 0),
      subject_id: legacySubjectId || (0 - Math.max(1, Number(row.group_subject_id || 0) || 1)),
      legacy_subject_id: legacySubjectId,
      subject_name: cleanText(row.subject_title || row.template_name, 160),
      subject_title: cleanText(row.subject_title || row.template_name, 160),
      target_group_numbers: activityType === 'lecture' ? [] : targetGroupNumbers,
      group_number: rowGroupNumber,
      group_label: buildGroupLabel(activityType === 'lecture', activityType === 'lecture' ? [] : targetGroupNumbers),
