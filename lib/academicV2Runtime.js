...
async function loadCourseScheduleRows(store, courseId, options = {}) {
  ...
  (rows || []).forEach((row) => {
    ...
    if (!legacySubjectId || !subject || !normalizePositiveInt(courseState.scope.legacy_course_id)) {
    ...
    scheduleRows.push({
      ...
      group_label: buildScheduleGroupLabel(activityType === 'lecture', activityType === 'lecture' ? [] : targetGroupNumbers),
      ...
      legacy_semester_id: normalizePositiveInt(courseState.term.legacy_semester_id),
    });
  });
