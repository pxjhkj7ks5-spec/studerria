...
function getScheduleDaySortValue(dayOfWeek) {
  const dayOrder = {
    Monday: 1,
    Tuesday: 2,
    Wednesday: 3,
  ...
  return Number(dayOrder[String(dayOfWeek || '').trim()] || 99);
}
...
async function loadCourseScheduleRows(store, courseId, options = {}) {
  ...
  (rows || []).forEach((row) => {
    ...
    const { groupCount: effectiveGroupCount, defaultGroup: effectiveDefaultGroup } = resolveScheduleEntryGroupScope(
      row,
      (subject && subject.group_count) || row.group_count || 1,
      (subject && subject.default_group) || row.default_group || 1
    );
    ...
    const rowGroupCount = effectiveGroupCount;
    const rowDefaultGroup = effectiveDefaultGroup;
  ...
  return {
    ...
    scheduleRows: scheduleRows.sort((a, b) => {
      const byDay = getScheduleDaySortValue(a.day_of_week) - getScheduleDaySortValue(b.day_of_week);
