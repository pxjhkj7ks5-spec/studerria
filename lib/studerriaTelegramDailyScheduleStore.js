...
function createDailyScheduleStore(db) {
  return {
    ...
    async loadStudents(course) {
      return db.all(`
        SELECT u.id, u.full_name, u.role, u.group_id, u.course_id, u.schedule_group,
               ...
               , u.show_full_schedule, COALESCE(tag_pref.tag_enabled, TRUE) AS telegram_tag_enabled
        FROM users u
        JOIN academic_v2_groups g ON g.id = u.group_id
        LEFT JOIN telegram_tag_preferences tag_pref
          ON tag_pref.telegram_id = TRIM(u.telegram_id)
    ...
    async toggleTagPreference(telegramId) {
      const normalizedTelegramId = String(telegramId || '').trim();
      if (!/^[1-9][0-9]*$/.test(normalizedTelegramId)) {
        throw new Error('invalid_telegram_id');
      }
      return db.get(`
        INSERT INTO telegram_tag_preferences (telegram_id, tag_enabled)
        VALUES (?, FALSE)
        ON CONFLICT (telegram_id) DO UPDATE
        SET tag_enabled = NOT telegram_tag_preferences.tag_enabled,
            updated_at = NOW()
        RETURNING telegram_id, tag_enabled, updated_at
