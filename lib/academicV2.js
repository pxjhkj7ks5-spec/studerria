...
function buildRegistrationGroupReadinessState(row = {}, duplicateGroupCount = 1) {
  ...
  if (Math.max(0, Number(duplicateGroupCount || 0) || 0) !== 1) {
    blockingIssueCodes.push('duplicate_stage_campus');
  }
...
function resolveTeacherRegistrationBucket(rows = []) {
  const bucketRows = Array.isArray(rows) ? rows : [];
  const readyRows = bucketRows.filter((row) => buildRegistrationGroupReadinessState(row, 1).ready);
  const defaultRows = readyRows.filter((row) => row.is_teacher_registration_default === true);
...
async function listRegistrationReadyGroups(store) {
  ...
  const readyRows = (rows || []).filter((row) => (
    normalizeTrackKey(row.track_key, 'bachelor') !== 'teacher'
  ...
  teacherBuckets.forEach((bucket) => {
    ...
    if (resolution.chosenRow) {
      readyRows.push(resolution.chosenRow);
    }
  });
  return readyRows;
}
