...
// One-time reconciliation for the approved PLED year-2 autumn timetable.
// Only term-1 memberships are replaced; subjects, selections, grades and schedule rows stay intact.
async function alignPledYear2FirstTermCatalog(store) {
  ...
  const entries = source.entries.filter((entry) => entry.suggested_stage_number === 2 && entry.suggested_term_numbers.includes(1));
  if (entries.length !== 10) throw new Error('PLED_YEAR2_TIMETABLE_MISMATCH');
  return withStoreTransaction(store, async (tx) => {
    ...
    for (const program of programs) {
      const stage = await tx.get(`INSERT INTO academic_v2_program_stage_templates (program_id, stage_number)
        VALUES (?, 2) ON CONFLICT (program_id, stage_number) DO UPDATE SET program_id = EXCLUDED.program_id RETURNING id`, [program.id]);
      ...
      const groups = await tx.all(`SELECT g.id, t.id AS term_id FROM academic_v2_groups g
        ...
        WHERE c.program_id = ? AND c.admission_year = 2025 AND g.stage_number = 2`, [program.id]);
      ...
      await tx.run(`INSERT INTO history_log (actor_name, action, details, created_at)
        VALUES ('System', 'pled_year2_term1_catalog_align_2026', ?, ?)`,
      [JSON.stringify({ program_id: program.id, backup }), new Date().toISOString()]);
      const stageSubjects = [];
      ...
      for (const group of groups) {
        ...
        for (const stageSubject of stageSubjects) {
          ...
          if (!existing || !existing.legacy_subject_id || existing.is_required !== groupSubject.is_required || existing.is_visible !== groupSubject.is_visible
