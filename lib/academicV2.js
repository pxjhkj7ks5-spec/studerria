...
async function saveGroupSubject(store, payload = {}) {
  const result = await withStoreTransaction(store, async (tx) => {
    ...
    const existingGroupSubject = groupSubjectId
      ? await tx.get(
        'SELECT id, sort_order FROM academic_v2_group_subjects WHERE id = ? LIMIT 1',
        [groupSubjectId]
      )
      : null;
    const resolvedSortOrder = hasExplicitSortOrder
...
async function saveScheduleEntry(store, payload = {}) {
  const result = await withStoreTransaction(store, async (tx) => {
    ...
    if (scheduleEntryId) {
      ...
      savedRows.push(row);
      usedRowIds.add(scheduleEntryId);
    } else {
      ...
      savedRows.push(row);
      if (existingPrimaryRow && existingPrimaryRow.id) {
        usedRowIds.add(Number(existingPrimaryRow.id || 0));
      }
    }
    for (const weekNumber of weekNumbers) {
      ...
      if (existingWeekRow && existingWeekRow.id) {
        usedRowIds.add(Number(existingWeekRow.id || 0));
      }
    }
    return {
      row,
