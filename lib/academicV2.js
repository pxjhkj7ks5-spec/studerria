...
const { indexScheduleSubjects, normalizeScheduleSubjectName, parseScheduleImportText, preferScheduleSubjectScope } = require('./scheduleTextImport');
const {
...
function resolveSubjectActivityPresetTypes(presetKey) {
  ...
  if (normalizedPreset === 'seminar_only') return ['seminar'];
...
function buildFocusState(rawFocus = {}, data = {}) {
  ...
  const requestedTermId = normalizePositiveInt(rawFocus.termId);
  const requestedTerm = terms.find((item) => Number(item.id) === Number(requestedTermId || 0)) || null;
  const requestedGroupId = normalizePositiveInt(rawFocus.groupId)
    || normalizePositiveInt(requestedTerm && requestedTerm.group_id);
  const requestedGroup = groups.find((item) => Number(item.id) === Number(requestedGroupId || 0)) || null;
  const requestedCohortId = normalizePositiveInt(requestedGroup && requestedGroup.cohort_id)
    || normalizePositiveInt(rawFocus.cohortId);
  const requestedCohort = cohorts.find((item) => Number(item.id) === Number(requestedCohortId || 0)) || null;

  let programId = normalizePositiveInt(requestedGroup && requestedGroup.program_id)
    || normalizePositiveInt(requestedCohort && requestedCohort.program_id)
    || normalizePositiveInt(rawFocus.programId);
...
async function syncStageSubjectTermLinksTx(tx, stageSubjectTemplateId, termRowsByNumber = new Map(), requestedTermNumbers = [], options = {}) {
  ...
  const existingLinks = await tx.all(
    `
      SELECT
        link.stage_subject_template_id,
      ...
      ORDER BY stage_term.term_number ASC, link.stage_term_template_id ASC
    `,
    [normalizedStageSubjectTemplateId]
  );
  ...
  if (mode === 'exact') {
    for (const existingLink of existingLinksByTermNumber.values()) {
      ...
      await tx.run(
        `
          DELETE FROM academic_v2_program_stage_subject_terms
          WHERE stage_subject_template_id = ?
            AND stage_term_template_id = ?
        `,
        [existingLink.stage_subject_template_id, existingLink.stage_term_template_id]
      );
      removedCount += 1;
    }
  }
...
async function saveBachelorCatalogRow(store, payload = {}) {
  const result = await withStoreTransaction(store, async (tx) => {
    ...
    const requestedIsGeneral = normalizeBoolean(payload.is_general, true);
    const templateRow = await ensureBachelorCatalogSubjectTemplateTx(tx, entry);
...
async function listScheduleEntries(store) {
  const rows = await store.all(
    `
      ...
      LEFT JOIN rooms room ON room.id = se.room_id
  ...
  return (rows || []).map((row) => {
    ...
    return {
      ...
      room_id: normalizePositiveInt(row.room_id),
      room_label: cleanText(row.room_label || row.room_code, 120),
...
const ACADEMIC_DAY_NAMES = Object.freeze([
  'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday',
]);
const ACADEMIC_DELIVERY_MODES = new Set(['offline', 'online', 'mixed']);

async function listGroupDayFormats(store) {
  const rows = await store.all(
    `SELECT group_id, day_of_week, delivery_mode
     FROM academic_v2_group_day_formats
     ORDER BY group_id ASC, day_of_week ASC`
  );
  return (rows || []).map((row) => ({
    group_id: Number(row.group_id || 0),
    day_of_week: ACADEMIC_DAY_NAMES.includes(String(row.day_of_week || '')) ? String(row.day_of_week) : 'Monday',
    delivery_mode: ACADEMIC_DELIVERY_MODES.has(String(row.delivery_mode || '').toLowerCase())
      ? String(row.delivery_mode).toLowerCase()
      : 'offline',
  }));
}

async function saveGroupDayFormats(store, payload = {}) {
  const groups = await listGroups(store);
  const groupIds = new Set((groups || []).map((group) => Number(group.id || 0)).filter(Boolean));
  return withStoreTransaction(store, async (tx) => {
    let saved = 0;
    for (const [fieldName, rawValue] of Object.entries(payload || {})) {
      const match = String(fieldName).match(/^mode_(\d+)_(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)$/);
      if (!match) continue;
      const groupId = Number(match[1]);
      const dayName = match[2];
      if (!groupIds.has(groupId)) continue;
      const rawMode = String(rawValue || '').trim().toLowerCase();
      const deliveryMode = ACADEMIC_DELIVERY_MODES.has(rawMode) ? rawMode : 'offline';
      await tx.run(
        `INSERT INTO academic_v2_group_day_formats (group_id, day_of_week, delivery_mode, updated_at)
         VALUES (?, ?, ?, NOW())
         ON CONFLICT (group_id, day_of_week)
         DO UPDATE SET delivery_mode = EXCLUDED.delivery_mode, updated_at = NOW()`,
        [groupId, dayName, deliveryMode]
      );
      saved += 1;
    }
    return { saved };
...
async function loadAcademicSetupPage(store, focus = {}, options = {}) {
  try {
    ...
    const groupDayFormats = unwrap(settled[17], [], 'day formats');
...
async function ensureLegacyCourse(tx, groupId) {
  ...
  if (!legacyCourseId) {
    const unboundLegacyCourses = await tx.all(
      `
        SELECT course.id
        FROM program_admission_courses admission_course
        JOIN courses course ON course.id = admission_course.course_id
        LEFT JOIN academic_v2_groups bound_group ON bound_group.legacy_course_id = course.id
        WHERE admission_course.admission_id = ?
          AND COALESCE(admission_course.is_visible, TRUE) = TRUE
          AND bound_group.id IS NULL
          AND LOWER(COALESCE(NULLIF(TRIM(course.location), ''), 'kyiv')) = ?
          AND COALESCE(course.is_teacher_course, FALSE) = ?
        ORDER BY course.id ASC
      `,
      [legacyAdmissionId, normalizeCampusKey(group.campus_key, 'kyiv'), teacherCourse]
    );
    if ((unboundLegacyCourses || []).length === 1) {
      legacyCourseId = normalizePositiveInt(unboundLegacyCourses[0] && unboundLegacyCourses[0].id);
      await tx.run(
        ...
        const inserted = await tx.get(
          `
            INSERT INTO schedule_entries
              (subject_id, group_number, day_of_week, class_number, week_number, course_id, semester_id, lesson_type, room_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            RETURNING id
          `,
          [
            ...
            legacySemesterId,
            activityType,
            normalizePositiveInt(entry.room_id),
          ]
        );
...
function buildCompactCourseIdentity(cohort, sourceGroup = {}) {
  const campusKey = normalizeCampusKey(sourceGroup && sourceGroup.campus_key, 'kyiv');
  const campusLabel = campusKey === 'munich' ? 'Мюнхен' : 'Київ';
  const campusCode = campusKey === 'munich' ? 'M' : 'K';
  const admissionYear = Number(cohort && cohort.admission_year || new Date().getUTCFullYear());
  const shortYear = String(admissionYear).slice(-2).padStart(2, '0');
  return {
    label: `${shortYear} ${campusLabel}`,
    code: `P${shortYear}${campusCode}`,
  };
}

function buildPromotedCourseLabel(cohort, sourceGroup) {
  return buildCompactCourseIdentity(cohort, sourceGroup).label;
}
...
async function ensurePromotionTargetCourseTx(tx, cohort, sourceGroup, targetStageNumber) {
  ...
  const inserted = await tx.get(
    ...
    [
      ...
      normalizeCourseStageNumber(targetStageNumber, 1),
      normalizeCampusKey(sourceGroup.campus_key, 'kyiv'),
      buildCompactCourseIdentity(cohort, sourceGroup).code,
...
async function activateProgramTerm(store, payload = {}) {
  const result = await withStoreTransaction(store, async (tx) => {
    const programId = normalizePositiveInt(
      payload.program_id
      || payload.programId
      || payload.focus_program_id
      || payload.focusProgramId
    );
    if (!programId) {
      throw new Error('PROGRAM_REQUIRED');
    }
    const termNumber = normalizeAcademicTermNumber(payload.term_number || payload.termNumber, 1);
    const program = await tx.get(
      `
    ...
    const groupRows = await tx.all(
      `
        ...
        ORDER BY g.stage_number ASC, g.campus_key ASC, g.id ASC
      `,
      [programId]
    );
    const touchedGroupIds = [];
    const touchedTermIds = [];
    for (const groupRow of groupRows || []) {
      ...
      await tx.run(
        `
          UPDATE academic_v2_terms
          SET
            is_active = CASE WHEN term_number = ? THEN TRUE ELSE FALSE END,
            is_archived = CASE WHEN term_number = ? THEN FALSE ELSE is_archived END,
            updated_at = NOW()
          WHERE group_id = ?
            AND term_number IN (1, 2, 3)
        `,
        [termNumber, termNumber, groupId]
      );
      const activeTermRow = await tx.get(
        `
          SELECT id
          FROM academic_v2_terms
          WHERE group_id = ?
      ...
      const activeTermId = normalizePositiveInt(activeTermRow && activeTermRow.id);
      if (activeTermId) {
        touchedTermIds.push(activeTermId);
      }
    ...
    return {
      row: {
        program_id: Number(program.id || 0),
        track_key: normalizeTrackKey(program.track_key, 'bachelor'),
        term_number: termNumber,
        group_count: touchedGroupIds.length,
      },
      touchedGroupIds,
      touchedTermIds,
    };
...
async function setProgramTermStartDates(store, payload = {}) {
  const result = await withStoreTransaction(store, async (tx) => {
    const programId = normalizePositiveInt(
      payload.program_id
      || payload.programId
    ...
    const termStartDateByNumber = new Map();
    for (const termNumber of academicTermNumberList()) {
      const payloadValue = payload[`term_${termNumber}_start_date`];
      if (String(payloadValue || '').trim() === '') {
        continue;
      }
      const normalizedDate = normalizeDateString(payloadValue, null);
      if (!normalizedDate) {
        throw new Error('TERM_START_DATE_INVALID');
      }
      termStartDateByNumber.set(termNumber, normalizedDate);
    }
    if (!termStartDateByNumber.size) {
      throw new Error('TERM_START_DATE_REQUIRED');
    }

    const stageTemplateRows = await tx.all(
      `
    ...
    for (const stageTemplateRow of stageTemplateRows || []) {
      const stageNumber = normalizeCourseStageNumber(stageTemplateRow && stageTemplateRow.stage_number, 1);
      await ensureCanonicalStageTermTemplatesTx(tx, programId, stageNumber);
      for (const [termNumber, startDate] of termStartDateByNumber.entries()) {
        await tx.run(
          `
            UPDATE academic_v2_program_stage_term_templates
            ...
            WHERE stage_template_id = ?
              AND term_number = ?
          `,
          [startDate, normalizePositiveInt(stageTemplateRow && stageTemplateRow.id), termNumber]
        );
      }
    }
    ...
    const touchedGroupIds = [];
    const touchedTermNumbers = Array.from(termStartDateByNumber.keys()).sort((left, right) => left - right);
    for (const groupRow of groupRows || []) {
      const groupId = normalizePositiveInt(groupRow && groupRow.id);
      if (!groupId) {
        continue;
    ...
    return {
      row: {
        ...
        track_key: normalizeTrackKey(program.track_key, 'bachelor'),
        group_count: touchedGroupIds.length,
        term_numbers: touchedTermNumbers,
        term_start_dates: touchedTermNumbers.reduce((acc, termNumber) => {
          acc[String(termNumber)] = termStartDateByNumber.get(termNumber) || null;
          return acc;
        }, {}),
      },
      touchedGroupIds,
      touchedTermNumbers,
      stageTemplateCount: Array.isArray(stageTemplateRows) ? stageTemplateRows.length : 0,
    };
  });
  return result;
}
...
async function applyStageTemplateToGroup(store, groupId) {
  const result = await withStoreTransaction(store, async (tx) => {
    ...
    if (
      normalizeTrackKey(row.track_key, 'bachelor') === 'bachelor'
      && await hasAcademicV2StageTemplateSchemaTx(tx)
    ) {
      await syncBachelorCatalogSourceTx(tx, {
        program_id: Number(row.program_id || 0),
...
function createScheduleImportError(message, code = 'SCHEDULE_IMPORT_INVALID') {
  const error = new Error(code);
  error.userMessage = cleanText(message, 240);
  return error;
}

async function createCohortIntake(store, payload = {}) {
  const result = await withStoreTransaction(store, async (tx) => {
    const programId = normalizePositiveInt(payload.program_id || payload.programId);
    const admissionYear = normalizePositiveInt(payload.admission_year || payload.admissionYear);
    if (!programId) {
      throw new Error('PROGRAM_REQUIRED');
    }
    if (!admissionYear || admissionYear < 2000 || admissionYear > 2100) {
      throw new Error('COHORT_YEAR_INVALID');
    }
    const program = await tx.get(
      `
        SELECT id, track_key, name
    ...
    const existingCohort = await tx.get(
      `
        SELECT *
        FROM academic_v2_cohorts
        WHERE program_id = ?
    ...
    const cohort = existingCohort
      ...
      : await tx.get(
        `
          ...
          RETURNING *
        `,
        [programId, admissionYear, `Набір ${admissionYear}`]
      );
    await ensureLegacyAdmission(tx, Number(cohort.id || 0));
    const stageNumber = normalizeCourseStageNumber(cohort.current_stage_number, 1);

    const campusMode = String(payload.campus_key || payload.campus || 'kyiv').trim().toLowerCase();
    const campusKeys = campusMode === 'both'
      ? ['kyiv', 'munich']
      : [normalizeCampusKey(campusMode, 'kyiv')];
    const sourceKey = normalizeBachelorCatalogSourceKey(
      payload.bachelor_catalog_source
      || payload.bachelor_catalog_source_key
      || DEFAULT_BACHELOR_CATALOG_SOURCE_KEY
    ...
    if (
      normalizeTrackKey(program.track_key, 'bachelor') === 'bachelor'
      && await hasAcademicV2StageTemplateSchemaTx(tx)
    ) {
      await syncBachelorCatalogSourceTx(tx, {
        program_id: programId,
    ...
    for (const campusKey of campusKeys) {
      const identity = buildCompactCourseIdentity(cohort, { campus_key: campusKey });
      const existingGroup = await tx.get(
        `
          SELECT *
          FROM academic_v2_groups
          ...
          LIMIT 1
        `,
        [Number(cohort.id || 0), stageNumber, campusKey]
      );
      const group = existingGroup
        ...
        : await tx.get(
          `
            ...
            RETURNING *
          `,
          [Number(cohort.id || 0), stageNumber, campusKey, identity.code, identity.label]
        );
      const needsConfiguredStructure = !existingGroup
        || await groupNeedsStageTemplateCoverageTx(tx, Number(group.id || 0));
      if (needsConfiguredStructure) {
        await applyConfiguredStructureToGroupTx(tx, Number(group.id || 0), {
          replaceExisting: !existingGroup,
          sourceKey,
        });
      }
      groups.push({
    ...
    return {
      programId,
      cohortId: Number(cohort.id || 0),
      groupIds: groups.map((group) => Number(group.id || 0)).filter(Boolean),
      row: groups[0] || null,
      restoredExisting: Boolean(existingCohort),
    };
  });
  return {
    ...result,
    ...(await runProjectionSyncSafely(store, result.groupIds, 'academicV2.createCohortIntake')),
  };
}

function normalizeImportedRoomLabel(value) {
  return cleanText(value, 120).toLocaleLowerCase('uk-UA');
}

async function ensureImportedScheduleRooms(store, groupId, entries = []) {
  const roomLabels = Array.from(new Map(
    (entries || [])
      .map((entry) => cleanText(entry && entry.roomLabel, 120))
      .filter(Boolean)
      .map((label) => [normalizeImportedRoomLabel(label), label])
  ).entries());
  if (!roomLabels.length) return new Map();

  return withStoreTransaction(store, async (tx) => {
    const group = await tx.get(
      `
        SELECT id, legacy_course_id, campus_key
        FROM academic_v2_groups
        WHERE id = ?
        LIMIT 1
      `,
    ...
    if (!group) {
      throw createScheduleImportError('Вибраний курс не знайдено.', 'SCHEDULE_SCOPE_MISMATCH');
    }
    const legacyCourseId = normalizePositiveInt(group.legacy_course_id)
      || (await ensureLegacyCourse(tx, groupId)).legacyCourseId;
    const campus = normalizeCampusKey(group.campus_key, 'kyiv');
    const resolved = new Map();

    for (const [key, label] of roomLabels) {
      let room = await tx.get(
        `
          SELECT id
          FROM rooms
          WHERE course_id = ?
            AND LOWER(TRIM(COALESCE(NULLIF(label, ''), code, ''))) = ?
          ORDER BY is_active DESC, id ASC
          LIMIT 1
        `,
        [legacyCourseId, key]
      ...
      if (!room) {
        const isOnline = /^(online|онлайн)$/i.test(label);
        room = await tx.get(
          `
            INSERT INTO rooms
              (course_id, campus, code, label, room_type, is_active, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, TRUE, NOW(), NOW())
            RETURNING id
          `,
          [legacyCourseId, campus, label, label, isOnline ? 'online' : 'classroom']
        );
      }
      resolved.set(key, normalizePositiveInt(room && room.id));
    }
    return resolved;
  });
}

async function importScheduleText(store, payload = {}) {
  const groupId = normalizePositiveInt(payload.group_id || payload.focus_group_id || payload.groupId);
  const termId = normalizePositiveInt(payload.term_id || payload.focus_term_id || payload.termId);
  if (!groupId || !termId) {
    throw createScheduleImportError('Спочатку виберіть курс і семестр для імпорту.');
  }

  const parsed = parseScheduleImportText(payload.schedule_text || payload.text || '');
  if (parsed.errors.length) {
    const firstErrors = parsed.errors.slice(0, 3).map((item) => (
      `${item.lineNumber ? `Рядок ${item.lineNumber}: ` : ''}${item.message}`
    ));
    throw createScheduleImportError(firstErrors.join(' · '));
  }

  const terms = await listTerms(store);
  const term = (terms || []).find((item) => Number(item.id || 0) === termId);
  if (!term || Number(term.group_id || 0) !== groupId) {
    throw createScheduleImportError('Вибраний семестр не належить цьому курсу.', 'SCHEDULE_SCOPE_MISMATCH');
  }

  parsed.entries.forEach((entry) => {
    if (entry.weekNumbers.some((weekNumber) => weekNumber > Number(term.weeks_count || 0))) {
      throw createScheduleImportError(
        `Рядок ${entry.lineNumber}: семестр має ${term.weeks_count} тижнів, але вказано більший номер.`
      );
    }
  });

  // Materialize the current bachelor catalog structure before matching names.
  await applyStageTemplateToGroup(store, groupId);

  let [subjects, activities, subjectTemplates] = await Promise.all([
    listGroupSubjects(store),
    listGroupSubjectActivities(store),
    listSubjectTemplates(store),
  ]);
  let scopedSubjects = (subjects || []).filter((item) => Number(item.group_id || 0) === groupId);
  let scopedActivities = (activities || []).filter((item) => Number(item.group_id || 0) === groupId);
  const templateRowsByKey = indexScheduleSubjects(subjectTemplates, ['name']);
  const requestedByKey = new Map();
  parsed.entries.forEach((entry) => {
    const key = normalizeScheduleSubjectName(entry.subject);
    if (!requestedByKey.has(key)) requestedByKey.set(key, []);
    requestedByKey.get(key).push(entry);
  });

  for (const [subjectKey, requestedEntries] of requestedByKey.entries()) {
    let subjectMatches = preferScheduleSubjectScope(indexScheduleSubjects(scopedSubjects, ['title', 'template_name']).get(subjectKey) || [], termId);
    if (!subjectMatches.length) {
      const templateMatches = templateRowsByKey.get(subjectKey) || [];
      if (templateMatches.length !== 1) {
        const requestedName = requestedEntries[0] && requestedEntries[0].subject;
        const reason = templateMatches.length
          ? `назва предмета «${requestedName}» неоднозначна у каталозі`
          : `предмет «${requestedName}» не знайдено у бакалаврському каталозі`;
        throw createScheduleImportError(`Рядок ${requestedEntries[0].lineNumber}: ${reason}.`);
      }
      const inferredGroupCount = Math.max(1, ...requestedEntries.flatMap((entry) => (
        entry.activityType === 'lecture' ? [1] : entry.targetGroupNumbers
      )));
      const created = await saveGroupSubject(store, {
        group_id: groupId,
        subject_template_id: Number(templateMatches[0].id || 0),
        title: templateMatches[0].name,
        group_count: inferredGroupCount,
        default_group: 1,
        term_ids: [termId],
        is_visible: true,
        is_required: true,
      ...
      subjects = await listGroupSubjects(store);
      scopedSubjects = subjects.filter((item) => Number(item.group_id || 0) === groupId);
      subjectMatches = scopedSubjects.filter((subject) => Number(subject.id || 0) === Number(created.row.id || 0));
    }
    if (subjectMatches.length !== 1) {
      throw createScheduleImportError(
        `Рядок ${requestedEntries[0].lineNumber}: назва предмета «${requestedEntries[0].subject}» неоднозначна.`
      );
    }
    let subject = subjectMatches[0];
    const explicitGroupCount = Math.max(1, ...requestedEntries.flatMap((entry) => (
      entry.activityType === 'lecture' ? [1] : entry.targetGroupNumbers
    )));
    if (explicitGroupCount > Number(subject.group_count || 1)) {
      await saveGroupSubject(store, {
        ...subject,
        group_subject_id: subject.id,
        group_id: groupId,
        subject_template_id: subject.subject_template_id,
        group_count: explicitGroupCount,
        term_ids: subject.term_ids,
        teacher_ids: subject.teacher_ids,
      });
      subjects = await listGroupSubjects(store);
      scopedSubjects = subjects.filter((item) => Number(item.group_id || 0) === groupId);
      subject = scopedSubjects.find((item) => Number(item.id || 0) === Number(subject.id || 0)) || subject;
    }
    activities = await listGroupSubjectActivities(store);
    scopedActivities = activities.filter((item) => Number(item.group_id || 0) === groupId);
    for (const activityType of new Set(requestedEntries.map((entry) => entry.activityType))) {
      const activityExists = scopedActivities.some((item) => (
        Number(item.group_subject_id || 0) === Number(subject.id || 0)
        && String(item.activity_type || '') === activityType
      ));
      if (!activityExists) {
        await saveGroupSubjectActivity(store, {
          group_subject_id: subject.id,
          activity_type: activityType,
        });
      }
  ...
  [subjects, activities] = await Promise.all([
    listGroupSubjects(store),
    listGroupSubjectActivities(store),
  ]);
  ...
  scopedSubjects = (subjects || []).filter((item) => Number(item.group_id || 0) === groupId);
  scopedActivities = (activities || []).filter((item) => Number(item.group_id || 0) === groupId);
  const subjectRowsByKey = indexScheduleSubjects(scopedSubjects, ['title', 'template_name']);

  const resolvedEntries = parsed.entries.map((entry) => {
    const subjectMatches = preferScheduleSubjectScope(subjectRowsByKey.get(normalizeScheduleSubjectName(entry.subject)) || [], termId);
    if (subjectMatches.length !== 1) {
      const reason = subjectMatches.length
        ? `назва предмета «${entry.subject}» неоднозначна`
        : `предмет «${entry.subject}» не знайдено у вибраному курсі`;
      throw createScheduleImportError(`Рядок ${entry.lineNumber}: ${reason}.`);
    }
    const subject = subjectMatches[0];
    const activity = scopedActivities.find((item) => (
      Number(item.group_subject_id || 0) === Number(subject.id || 0)
      && String(item.activity_type || '') === entry.activityType
    ));
    if (!activity) {
      throw createScheduleImportError(
        `Рядок ${entry.lineNumber}: для «${entry.subject}» не налаштовано тип заняття «${entry.activityType}».`
      );
    }
    const weekNumbers = entry.weekNumbers;
    const groupCount = Math.max(1, Number(activity.group_count || subject.group_count || 1));
    const targetGroupNumbers = entry.activityType === 'lecture'
      ? []
      : (entry.allGroups
        ? Array.from({ length: groupCount }, (_item, index) => index + 1)
        : entry.targetGroupNumbers);
    if (targetGroupNumbers.some((groupNumber) => groupNumber > groupCount)) {
      throw createScheduleImportError(
        `Рядок ${entry.lineNumber}: у предмета «${entry.subject}» налаштовано лише ${groupCount} груп.`
      );
    }
    return {
      group_subject_activity_id: Number(activity.id),
      term_id: termId,
      day_of_week: entry.dayOfWeek,
      class_number: entry.classNumber,
      week_numbers: weekNumbers,
      target_group_numbers: targetGroupNumbers,
      group_number: targetGroupNumbers[0] || 1,
      room_label: entry.roomLabel,
    };
  });

  const roomIdsByLabel = await ensureImportedScheduleRooms(store, groupId, parsed.entries);
  resolvedEntries.forEach((entry) => {
    entry.room_id = roomIdsByLabel.get(normalizeImportedRoomLabel(entry.room_label)) || null;
    delete entry.room_label;
  });

  const replaceExisting = normalizeBoolean(payload.replace_existing, false);
  if (replaceExisting) {
    await clearScheduleEntriesForTerm(store, { group_id: groupId, term_id: termId });
  }
  ...
  try {
    for (const entry of resolvedEntries) {
      const result = await saveScheduleEntryRows(store, entry);
      savedRows += Array.isArray(result && result.rows) ? result.rows.length : 0;
    }
  } finally {
    // Also reconcile committed rows when a later row fails. Each row keeps its
    // existing transaction, but a full course sync runs only once per batch.
    projectionResult = await runProjectionSyncSafely(store, groupId, 'academicV2.importScheduleText');
  }

  return {
    groupId,
    termId,
    importedEntries: resolvedEntries.length,
    savedRows,
...
async function saveScheduleEntryRows(store, payload = {}) {
  return withStoreTransaction(store, async (tx) => {
    ...
    const roomId = normalizePositiveInt(payload.room_id);
    ...
    if (roomId) {
      const expectedRoomCourseId = normalizePositiveInt(activity.legacy_course_id)
        || (await ensureLegacyCourse(tx, Number(activity.group_id || 0))).legacyCourseId;
      ...
      if (!room || (
        normalizePositiveInt(room.course_id)
        && normalizePositiveInt(room.course_id) !== normalizePositiveInt(expectedRoomCourseId)
      )) {
        throw new Error('SCHEDULE_ROOM_INVALID');
      }
    }
        ...
        : tx.get(
          `
            INSERT INTO academic_v2_schedule_entries
              (group_subject_id, group_subject_activity_id, term_id, group_number, target_group_numbers, day_of_week, class_number, week_number, lesson_type, room_id, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
            RETURNING *
          `,
          [
...
async function saveScheduleEntry(store, payload = {}) {
  const result = await saveScheduleEntryRows(store, payload);
  return {
    ...result,
...
async function clearScheduleEntriesForSubjects(store, payload = {}) {
  const termId = normalizePositiveInt(payload.term_id || payload.focus_term_id || payload.termId);
  const requestedGroupId = normalizePositiveInt(payload.group_id || payload.focus_group_id || payload.groupId);
  const requestedSubjectIds = normalizeIdArray(
    payload.group_subject_ids || payload.group_subject_id || payload.subject_ids || payload.subject_id
  );
  if (!termId) {
    throw new Error('TERM_NOT_FOUND');
  }
  if (!requestedSubjectIds.length) {
    throw new Error('SCHEDULE_SUBJECT_SELECTION_REQUIRED');
  }
  ...
  const result = await withStoreTransaction(store, async (tx) => {
    ...
    const subjectPlaceholders = requestedSubjectIds.map(() => '?').join(', ');
    const scopedSubjects = await tx.all(
      `
        SELECT id
        FROM academic_v2_group_subjects
        WHERE group_id = ?
          AND id IN (${subjectPlaceholders})
        ORDER BY id ASC
      `,
      [groupId, ...requestedSubjectIds]
    );
    const scopedSubjectIds = normalizeIdArray((scopedSubjects || []).map((subject) => subject.id));
    if (scopedSubjectIds.length !== requestedSubjectIds.length) {
      throw new Error('SCHEDULE_SCOPE_MISMATCH');
    }

    const scopedPlaceholders = scopedSubjectIds.map(() => '?').join(', ');
    const rows = await tx.all(
      `
        SELECT se.id
        FROM academic_v2_schedule_entries se
        ...
        WHERE se.term_id = ?
          AND gs.group_id = ?
          AND se.group_subject_id IN (${scopedPlaceholders})
        ORDER BY se.id ASC
      `,
      [termId, groupId, ...scopedSubjectIds]
    );
  ...
  return {
    ...result,
    ...(await runProjectionSyncSafely(store, result && result.groupId, 'academicV2.clearScheduleEntriesForSubjects')),
  };
}
...
async function ensurePledStudentStructure(store) {
  const cohortBlueprints = [
    { admissionYear: 2024, stageNumber: 3 },
    { admissionYear: 2025, stageNumber: 2 },
    { admissionYear: 2026, stageNumber: 1 },
  ];
  const programs = await listPrograms(store);
  const bachelorPrograms = (programs || [])
    .filter((program) => normalizeTrackKey(program.track_key, 'bachelor') === 'bachelor')
    .sort((left, right) => {
      const leftIsPled = /\bpled\b|плед/i.test(`${left.code || ''} ${left.name || ''}`) ? 1 : 0;
      const rightIsPled = /\bpled\b|плед/i.test(`${right.code || ''} ${right.name || ''}`) ? 1 : 0;
      if (leftIsPled !== rightIsPled) return rightIsPled - leftIsPled;
      return Number(right.group_count || 0) - Number(left.group_count || 0);
    });
  const existingProgram = bachelorPrograms[0] || null;
  const programResult = await saveProgram(store, {
    program_id: existingProgram && existingProgram.id,
    track_key: 'bachelor',
    code: 'PLED',
    name: 'ПЛЕД',
    sort_order: 10,
  ...
  const programId = Number(programResult && programResult.row && programResult.row.id || 0);
  if (!programId) throw new Error('PROGRAM_NOT_FOUND');

  const ensuredCohorts = [];
  const ensuredGroups = [];
  for (const blueprint of cohortBlueprints) {
    const cohorts = await listCohorts(store);
    const existingCohort = (cohorts || []).find((cohort) => (
      Number(cohort.program_id || 0) === programId
      && Number(cohort.admission_year || 0) === blueprint.admissionYear
    ));
    const cohortResult = await saveCohort(store, {
      cohort_id: existingCohort && existingCohort.id,
      program_id: programId,
      admission_year: blueprint.admissionYear,
      label: `Набір ${blueprint.admissionYear}`,
      current_stage_number: blueprint.stageNumber,
      is_active: true,
    });
    const cohortId = Number(cohortResult && cohortResult.row && cohortResult.row.id || 0);
    ensuredCohorts.push(cohortId);

    const groups = await listGroups(store);
    const existingGroup = (groups || []).find((group) => (
      Number(group.cohort_id || 0) === cohortId
      && Number(group.stage_number || 0) === blueprint.stageNumber
      && normalizeCampusKey(group.campus_key, 'kyiv') === 'kyiv'
    ));
    let groupId = Number(existingGroup && existingGroup.id || 0);
    if (!groupId) {
      const groupResult = await saveGroup(store, {
        cohort_id: cohortId,
        stage_number: blueprint.stageNumber,
        campus_key: 'kyiv',
        code: buildCompactCourseIdentity({ admission_year: blueprint.admissionYear }, { campus_key: 'kyiv' }).code,
        label: buildCompactCourseIdentity({ admission_year: blueprint.admissionYear }, { campus_key: 'kyiv' }).label,
        is_active: true,
      });
      groupId = Number(groupResult && groupResult.row && groupResult.row.id || 0);
    }
    ensuredGroups.push(groupId);
  }

  return {
    programId,
    cohortIds: ensuredCohorts,
    groupIds: ensuredGroups,
    cohortId: ensuredCohorts[ensuredCohorts.length - 1] || null,
    groupId: ensuredGroups[ensuredGroups.length - 1] || null,
  };
}

// Reconcile one approved PLED autumn timetable without deleting subjects, selections,
// grades or schedule rows. Only the selected stage's term-1 memberships are replaced.
async function alignPledFirstTermCatalog(store, config = {}) {
  if (!store || typeof store.withTransaction !== 'function') throw new Error('TRANSACTION_REQUIRED');
  const stageNumber = normalizeCourseStageNumber(config.stageNumber, 1);
  const admissionYear = normalizePositiveInt(config.admissionYear);
  const expectedCount = normalizePositiveInt(config.expectedCount);
  const historyAction = cleanText(config.historyAction, 120);
  if (!admissionYear || !expectedCount || !historyAction) throw new Error('PLED_CATALOG_CONFIG_REQUIRED');
  const source = getBachelorCatalogSource(DEFAULT_BACHELOR_CATALOG_SOURCE_KEY);
  const entries = source.entries.filter((entry) => entry.suggested_stage_number === stageNumber && entry.suggested_term_numbers.includes(1));
  if (entries.length !== expectedCount) throw new Error(`PLED_YEAR${stageNumber}_TIMETABLE_MISMATCH`);
  return withStoreTransaction(store, async (tx) => {
    const programs = await tx.all(`SELECT id FROM academic_v2_programs
      WHERE track_key = 'bachelor' AND (UPPER(code) = 'PLED' OR name IN ('ПЛЕД', 'Політичне лідерство та економічна дипломатія'))`);
    const changed = [];
    for (const program of programs) {
      const stage = await tx.get(`INSERT INTO academic_v2_program_stage_templates (program_id, stage_number)
        VALUES (?, ?) ON CONFLICT (program_id, stage_number) DO UPDATE SET program_id = EXCLUDED.program_id RETURNING id`, [program.id, stageNumber]);
      const term = await tx.get(`INSERT INTO academic_v2_program_stage_term_templates (stage_template_id, term_number, title, weeks_count, sort_order)
        VALUES (?, 1, '1 семестр', 15, 1) ON CONFLICT (stage_template_id, term_number)
        DO UPDATE SET stage_template_id = EXCLUDED.stage_template_id RETURNING id`, [stage.id]);
      const groups = await tx.all(`SELECT g.id, t.id AS term_id FROM academic_v2_groups g
        JOIN academic_v2_cohorts c ON c.id = g.cohort_id
        JOIN academic_v2_terms t ON t.group_id = g.id AND t.term_number = 1
        WHERE c.program_id = ? AND c.admission_year = ? AND g.stage_number = ?`, [program.id, admissionYear, stageNumber]);
      // Keep a recoverable record of the old links and flags before any removal.
      const backup = {
        stage_subjects: await tx.all('SELECT * FROM academic_v2_program_stage_subject_templates WHERE stage_template_id = ?', [stage.id]),
        stage_links: await tx.all('SELECT * FROM academic_v2_program_stage_subject_terms WHERE stage_term_template_id = ?', [term.id]),
        group_subjects: await tx.all('SELECT * FROM academic_v2_group_subjects WHERE group_id = ANY(?::int[])', [groups.map((g) => g.id)]),
        group_links: await tx.all('SELECT * FROM academic_v2_group_subject_terms WHERE term_id = ANY(?::int[])', [groups.map((g) => g.term_id)]),
        stage_activities: await tx.all(`SELECT a.* FROM academic_v2_program_stage_subject_activities a
          JOIN academic_v2_program_stage_subject_templates s ON s.id = a.stage_subject_template_id WHERE s.stage_template_id = ?`, [stage.id]),
        group_activities: await tx.all(`SELECT a.* FROM academic_v2_group_subject_activities a
          JOIN academic_v2_group_subjects s ON s.id = a.group_subject_id WHERE s.group_id = ANY(?::int[])`, [groups.map((g) => g.id)]),
      };
      await tx.run(`INSERT INTO history_log (actor_name, action, details, created_at)
        VALUES ('System', ?, ?, ?)`,
      [historyAction, JSON.stringify({ program_id: program.id, backup }), new Date().toISOString()]);
      const stageSubjects = [];
      for (const entry of entries) {
        const template = await ensureBachelorCatalogSubjectTemplateTx(tx, entry);
        const stageSubject = await tx.get(`INSERT INTO academic_v2_program_stage_subject_templates
          (stage_template_id, subject_template_id, title, group_count, is_required)
        VALUES (?, ?, ?, ?, ?) ON CONFLICT (stage_template_id, subject_template_id)
        DO UPDATE SET title = EXCLUDED.title, is_visible = TRUE, group_count = EXCLUDED.group_count,
            default_group = 1, is_required = EXCLUDED.is_required
          RETURNING *`, [stage.id, template.id, entry.display_title, entry.default_group_count,
          entry.default_flags.is_required]);
        await tx.run(`INSERT INTO academic_v2_program_stage_subject_terms (stage_subject_template_id, stage_term_template_id)
          VALUES (?, ?) ON CONFLICT DO NOTHING`, [stageSubject.id, term.id]);
        await ensureStageSubjectActivityPresetTx(tx, stageSubject.id, entry.default_activity_preset);
        if (entry.default_activity_preset === 'seminar_only') {
          await tx.run("DELETE FROM academic_v2_program_stage_subject_activities WHERE stage_subject_template_id = ? AND activity_type <> 'seminar'", [stageSubject.id]);
        }
        stageSubjects.push({ ...stageSubject, activityPreset: entry.default_activity_preset });
      }
      await tx.run(`DELETE FROM academic_v2_program_stage_subject_terms link
        USING academic_v2_program_stage_subject_templates subject
        WHERE link.stage_subject_template_id = subject.id AND subject.stage_template_id = ?
          AND link.stage_term_template_id = ? AND NOT (subject.id = ANY(?::int[]))`,
      [stage.id, term.id, stageSubjects.map((subject) => subject.id)]);
      for (const group of groups) {
        const groupSubjectIds = [];
        for (const stageSubject of stageSubjects) {
          const existing = await tx.get('SELECT * FROM academic_v2_group_subjects WHERE group_id = ? AND subject_template_id = ?',
            [group.id, stageSubject.subject_template_id]);
          const groupSubject = await tx.get(`INSERT INTO academic_v2_group_subjects
            (group_id, subject_template_id, title, group_count, default_group, is_visible, is_required, is_general, show_in_teamwork)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (group_id, subject_template_id)
            DO UPDATE SET title = EXCLUDED.title, is_visible = EXCLUDED.is_visible, is_required = EXCLUDED.is_required,
              group_count = EXCLUDED.group_count, default_group = EXCLUDED.default_group RETURNING *`,
          [group.id, stageSubject.subject_template_id, stageSubject.title, stageSubject.group_count,
            stageSubject.default_group, stageSubject.is_visible, stageSubject.is_required, stageSubject.is_general, stageSubject.show_in_teamwork]);
          await tx.run(`INSERT INTO academic_v2_group_subject_terms (group_subject_id, term_id)
            VALUES (?, ?) ON CONFLICT DO NOTHING`, [groupSubject.id, group.term_id]);
          await ensureGroupSubjectActivityPresetTx(tx, groupSubject.id, stageSubject.activityPreset);
          if (stageSubject.activityPreset === 'seminar_only') {
            const scheduledOtherType = await tx.get(`SELECT e.id FROM academic_v2_schedule_entries e
              JOIN academic_v2_group_subject_activities a ON a.id = e.group_subject_activity_id
              WHERE a.group_subject_id = ? AND a.activity_type <> 'seminar' LIMIT 1`, [groupSubject.id]);
            if (scheduledOtherType) throw new Error('PLED_LANGUAGE_NON_SEMINAR_SCHEDULE_REQUIRES_REVIEW');
            // Delete only unused activity definitions: their FK otherwise cascades into the timetable.
            await tx.run("DELETE FROM academic_v2_group_subject_activities WHERE group_subject_id = ? AND activity_type <> 'seminar'", [groupSubject.id]);
          }
          // Project only changed/missing subjects; do not rebuild schedules or other terms.
          if (!existing || !existing.legacy_subject_id || cleanText(existing.title, 160) !== cleanText(groupSubject.title, 160)
            || existing.is_required !== groupSubject.is_required || existing.is_visible !== groupSubject.is_visible
            || existing.group_count !== groupSubject.group_count || existing.default_group !== groupSubject.default_group) {
            await ensureLegacySubject(tx, groupSubject.id);
          }
          groupSubjectIds.push(groupSubject.id);
        }
        await tx.run(`DELETE FROM academic_v2_group_subject_terms link USING academic_v2_group_subjects subject
          WHERE link.group_subject_id = subject.id AND subject.group_id = ?
            AND link.term_id = ? AND NOT (subject.id = ANY(?::int[]))`, [group.id, group.term_id, groupSubjectIds]);
      }
      changed.push({ programId: program.id, groups: groups.length, subjects: stageSubjects.length });
    }
    return changed;
  });
}

async function alignPledYear2FirstTermCatalog(store) {
  return alignPledFirstTermCatalog(store, {
    stageNumber: 2,
    admissionYear: 2025,
    expectedCount: 10,
    historyAction: 'pled_year2_term1_catalog_align_2026',
  });
}

async function alignPledYear1FirstTermCatalog(store) {
  return alignPledFirstTermCatalog(store, {
    stageNumber: 1,
    admissionYear: 2026,
    expectedCount: 9,
    historyAction: 'pled_year1_term1_catalog_align_2026',
  });
}

async function alignPledYear3FirstTermCatalog(store) {
  return alignPledFirstTermCatalog(store, {
    stageNumber: 3,
    admissionYear: 2024,
    expectedCount: 9,
    historyAction: 'pled_year3_term1_catalog_align_2026',
  });
}
...
module.exports = {
  alignPledYear1FirstTermCatalog,
  alignPledYear2FirstTermCatalog,
  alignPledYear3FirstTermCatalog,
  listGroupDayFormats,
  saveGroupDayFormats,
  ...
  clearScheduleEntriesForSubjects,
  ensurePledStudentStructure,
