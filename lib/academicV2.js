...
function buildBachelorCatalogStatus(entry = {}, stageSubject = null, currentTermNumbers = []) {
  ...
  const suggestedStageNumber = normalizePositiveInt(entry.suggested_stage_number);
  const suggestedTerms = normalizeIdArray(entry.suggested_term_numbers || []).filter((value) => value === 1 || value === 2);
...
async function ensureBachelorStageTermTemplatesTx(tx, programId, stageNumber) {
  ...
  const termRowsByNumber = new Map(
    (existingTermRows || []).map((row) => [Math.max(1, Number(row.term_number || 0) || 1), { ...row, id: Number(row.id || 0) }])
  );
  for (const termNumber of [1, 2]) {
    if (termRowsByNumber.has(termNumber)) {
      continue;
    }
    const row = await tx.get(
      `
        ...
        VALUES (?, ?, ?, NULL, 16, ?, ?, NOW(), NOW())
        RETURNING id, term_number
      `,
      [stageTemplate.id, termNumber, `Term ${termNumber}`, termNumber === 1, termNumber]
    );
...
async function syncBachelorCatalogSource(store, payload = {}) {
  return withStoreTransaction(store, async (tx) => {
    ...
    for (const entry of source.entries || []) {
      ...
      const existingRow = await findBachelorCatalogStageSubjectByTemplateTx(tx, {
        programId,
      ...
      const selectedTermNumbers = normalizeIdArray(entry.suggested_term_numbers || []).filter((value) => value === 1 || value === 2);
      for (const termNumber of selectedTermNumbers) {
...
async function saveBachelorCatalogRow(store, payload = {}) {
  return withStoreTransaction(store, async (tx) => {
    ...
    const requestedTermNumbers = normalizeIdArray(payload.term_numbers || []).filter((value) => value === 1 || value === 2);
    ...
    if (existingRow && Number(existingRow.id || 0)) {
      row = await tx.get(
        ...
        [
          stageTemplate.id,
          normalizeBoolean(payload.is_required, normalizeBoolean(entry.default_flags && entry.default_flags.is_required, true)),
          normalizeBoolean(payload.is_general, normalizeBoolean(entry.default_flags && entry.default_flags.is_general, true)),
          existingRow.id,
        ]
      );
