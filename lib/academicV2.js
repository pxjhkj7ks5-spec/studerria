...
const { parseScheduleImportText } = require('./scheduleTextImport');
const {
...
async function importScheduleText(store, payload = {}) {
  ...
  const templateRowsByKey = new Map();
  (subjectTemplates || []).forEach((template) => {
    const key = normalizeSubjectTemplateName(template.name);
    if (!key) return;
    if (!templateRowsByKey.has(key)) templateRowsByKey.set(key, []);
    templateRowsByKey.get(key).push(template);
  });
  const requestedByKey = new Map();
  parsed.entries.forEach((entry) => {
    const key = normalizeSubjectTemplateName(entry.subject);
  ...
  for (const [subjectKey, requestedEntries] of requestedByKey.entries()) {
    let subjectMatches = scopedSubjects.filter((subject) => (
      normalizeSubjectTemplateName(subject.title) === subjectKey
      || normalizeSubjectTemplateName(subject.template_name) === subjectKey
    ));
  ...
  scopedSubjects = (subjects || []).filter((item) => Number(item.group_id || 0) === groupId);
  scopedActivities = (activities || []).filter((item) => Number(item.group_id || 0) === groupId);
  const subjectRowsByKey = new Map();
  scopedSubjects.forEach((subject) => {
    [subject.title, subject.template_name].forEach((name) => {
      const key = normalizeSubjectTemplateName(name);
      if (!key) return;
      if (!subjectRowsByKey.has(key)) subjectRowsByKey.set(key, []);
      if (!subjectRowsByKey.get(key).some((item) => Number(item.id) === Number(subject.id))) {
        subjectRowsByKey.get(key).push(subject);
      }
    });
  });

  const resolvedEntries = parsed.entries.map((entry) => {
    const subjectMatches = subjectRowsByKey.get(normalizeSubjectTemplateName(entry.subject)) || [];
    if (subjectMatches.length !== 1) {
