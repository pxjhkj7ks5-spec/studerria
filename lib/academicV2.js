...
const { indexScheduleSubjects, normalizeScheduleSubjectName, parseScheduleImportText } = require('./scheduleTextImport');
const {
...
async function importScheduleText(store, payload = {}) {
  ...
  for (const [subjectKey, requestedEntries] of requestedByKey.entries()) {
    let subjectMatches = indexScheduleSubjects(scopedSubjects, ['title', 'template_name']).get(subjectKey) || [];
  ...
  const resolvedEntries = parsed.entries.map((entry) => {
    const subjectMatches = subjectRowsByKey.get(normalizeScheduleSubjectName(entry.subject)) || [];
    if (subjectMatches.length !== 1) {
