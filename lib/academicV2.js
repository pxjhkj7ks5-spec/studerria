...
async function importScheduleText(store, payload = {}) {
  ...
  const [terms, subjects, activities] = await Promise.all([
    listTerms(store),
    listGroupSubjects(store),
    listGroupSubjectActivities(store),
  ]);
  ...
  const scopedSubjects = (subjects || []).filter((item) => Number(item.group_id || 0) === groupId);
  const scopedActivities = (activities || []).filter((item) => Number(item.group_id || 0) === groupId);
  ...
  const resolvedEntries = parsed.entries.map((entry) => {
    ...
    const weekNumbers = entry.weekNumbers.filter((weekNumber) => weekNumber <= Number(term.weeks_count || 0));
    if (weekNumbers.length !== entry.weekNumbers.length) {
      throw createScheduleImportError(
  ...
  for (const blueprint of cohortBlueprints) {
    ...
    if (!groupId) {
      const groupResult = await saveGroup(store, {
        ...
        campus_key: 'kyiv',
        code: `PLED-${blueprint.admissionYear}`,
        label: `ПЛЕД · ${blueprint.stageNumber} курс · набір ${blueprint.admissionYear}`,
        is_active: true,
      });
