      ...
      SELECT
        ...
        EXISTS (
          ...
          WHERE term.group_id = g.id
            AND COALESCE(term.is_archived, FALSE) = FALSE
        ...
        (
          ...
          WHERE subject.group_id = g.id
            AND COALESCE(subject.is_visible, TRUE) = TRUE
...
function buildRegistrationGroupReadinessState(row = {}, duplicateGroupCount = 1) {
  ...
  const issueCodes = [];
  if (Math.max(0, Number(duplicateGroupCount || 0) || 0) !== 1) {
    issueCodes.push('duplicate_stage_campus');
  }
  if (!normalizeBoolean(row.has_active_term, false)) {
    issueCodes.push('missing_active_term');
  }
  if (Math.max(0, Number(row.visible_mapped_subject_count || 0) || 0) < 1) {
    issueCodes.push('missing_visible_subjects');
  }
  if (missingCompatFields.length) {
    issueCodes.push('missing_compat_bridge');
  }
  return {
    ready: !issueCodes.length,
...
async function listRegistrationReadyGroups(store) {
  ...
  return (rows || []).filter((row) => (
    buildRegistrationGroupReadinessState(
      row,
      duplicateCounts.get(buildRegistrationGroupScopeKey(row)) || 0
    ).ready
  ));
}
...
async function listRegistrationGroupAuditIssues(store) {
  ...
  (rows || []).forEach((row) => {
    const scopeKey = buildRegistrationGroupScopeKey(row);
    const readiness = buildRegistrationGroupReadinessState(
      row,
      (buckets.get(scopeKey) || []).length
    );
    if (readiness.ready) {
      return;
    }
    const issueCodes = readiness.issue_codes.filter((code) => code !== 'duplicate_stage_campus');
    ...
    if (issueCodes.includes('missing_compat_bridge')) {
      issueCode = 'registration_group_missing_compat_bridge';
      severity = 'high';
      title = 'Registration academic group is missing compatibility bridge ids';
      summary = 'Registration can only use academic_v2 groups that still project required legacy ids for downstream compatibility.';
    } else if (issueCodes.includes('missing_active_term')) {
      issueCode = 'registration_group_missing_active_term';
      title = 'Registration academic group has no active term';
      summary = 'Registration stage-1 academic_v2 groups need one active term before subject selection can open.';
    } else if (issueCodes.includes('missing_visible_subjects')) {
      issueCode = 'registration_group_missing_visible_subjects';
      title = 'Registration academic group has no visible mapped subjects';
      summary = 'Registration stage-1 academic_v2 groups need at least one visible mapped subject before subject selection can open.';
    }
    issues.push({
...
async function listStaleStudyContextRows(store, limit = null) {
  const params = [];
  const limitClause = Number.isInteger(Number(limit)) && Number(limit) > 0 ? 'LIMIT ?' : '';
  if (limitClause) {
  ...
  return allRowsSafely(() => store.all(
    `
      ...
      WHERE COALESCE(context.is_active, TRUE) = TRUE
        AND NOT EXISTS (
          SELECT 1
          FROM academic_v2_groups g
...
async function listStaleProgramPresetRows(store, limit = null) {
  ...
  return allRowsSafely(() => store.all(
    `
      ...
      WHERE COALESCE(preset.is_active, TRUE) = TRUE
        AND (
          NOT EXISTS (
            SELECT 1
...
async function listStaleLegacyOfferingRows(store, limit = null) {
  ...
  return allRowsSafely(() => store.all(
    `
      ...
      WHERE COALESCE(offering.is_active, TRUE) = TRUE
...
async function loadAcademicSetupPage(store, focus = {}, options = {}) {
  ...
  const unwrap = (result, fallback, label) => {
    ...
    renderWarnings.push({
      section: label,
      message: normalizeRuntimeErrorMessage(result.reason && result.reason.message ? result.reason.message : result.reason),
    });
    return fallback;
  };
  ...
  const groupSubjects = unwrap(settled[7], [], 'group subjects');
  const scheduleEntries = unwrap(settled[8], [], 'schedule entries');
  const teachers = unwrap(settled[9], [], 'teachers');
  const users = unwrap(settled[10], [], 'assignable users');
  const summary = unwrap(settled[11], {
    programs_total: 0,
    cohorts_total: 0,
    groups_total: 0,
    terms_total: 0,
  ...
  const projectionHealth = unwrap(settled[12], {
    items: [],
    summary: {
      groups_total: 0,
      groups_healthy: 0,
  ...
  const auditSummary = unwrap(settled[13], {
...
async function clearGroupScheduleProjection(tx, groupId) {
  const scheduleRows = await tx.all(
    `
      SELECT se.id, se.legacy_schedule_entry_id
      FROM academic_v2_schedule_entries se
      JOIN academic_v2_group_subjects gs ON gs.id = se.group_subject_id
      WHERE gs.group_id = ?
    `,
...
async function applyStageTemplateToGroupTx(tx, groupId, { replaceExisting = false } = {}) {
  ...
  if (replaceExisting) {
    for (const existingSubject of existingSubjects || []) {
      ...
      const scheduleRows = await tx.all(
        'SELECT id, legacy_schedule_entry_id FROM academic_v2_schedule_entries WHERE group_subject_id = ?',
        [existingSubject.id]
      );
      for (const scheduleRow of scheduleRows || []) {
        const legacyScheduleEntryId = normalizePositiveInt(scheduleRow.legacy_schedule_entry_id);
      ...
      row = await tx.get(
        `
          ...
          SET
            ...
            admission_year = ?,
            label = ?,
            current_stage_number = COALESCE(?, current_stage_number),
            is_active = ?,
            updated_at = NOW()
          WHERE id = ?
          RETURNING *
