  ...
  await withStoreTransaction(store, async (tx) => {
    ...
    if (programId && programTrackKey === 'bachelor') {
      ...
      for (const groupRow of programGroups || []) {
        ...
        await applyStageTemplateToGroupTx(tx, groupId, { replaceExisting: false });
...
async function groupNeedsStageTemplateCoverageTx(tx, groupId) {
  ...
  for (const stageSubject of bundle.subjectTemplates || []) {
    ...
    const desiredTermNumbers = (
      Array.isArray(stageSubject.stage_term_template_ids) && stageSubject.stage_term_template_ids.length
        ? stageSubject.stage_term_template_ids
          .map((termTemplateId) => Number(stageTermNumberById.get(Number(termTemplateId || 0)) || 0))
          .filter((value) => academicTermNumberList().includes(value))
        : academicTermNumberList()
    ).sort((a, b) => a - b);
  ...
  const result = await withStoreTransaction(store, async (tx) => {
    try {
      ...
      if (Number(normalizedRow.id || 0) > 0) {
        ...
        if (
          normalizedProgramTrackKey === 'bachelor'
          && (forceTemplateRealignment || await groupNeedsStageTemplateCoverageTx(tx, Number(normalizedRow.id || 0)))
        ) {
          await applyStageTemplateToGroupTx(tx, Number(normalizedRow.id || 0), {
            replaceExisting: forceTemplateRealignment,
          });
        }
      }
      return {
