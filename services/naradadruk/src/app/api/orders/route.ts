...
export async function POST(request: Request) {
  ...
  try {
    const order = await prisma.$transaction(async (transaction) => {
      ...
      return transaction.order.create({
        data: {
          ...
          events: { create: { eventType: "created", toStatus: "new", actor: "website" } },
        },
    ...
    await prisma.order.update({ where: { id: order.id }, data: { notificationStatus: notification.status, notificationError: notification.error } })
      .catch((error) => console.error("[orders] notification status update failed", error));
    return NextResponse.json({ publicId: order.publicId, total: order.total }, { status: 201 });
  } catch (error) {
