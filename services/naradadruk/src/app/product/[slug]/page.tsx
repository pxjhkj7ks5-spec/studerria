...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  return (
    <main className="mx-auto w-full max-w-[1400px] px-4 py-8 md:px-6 md:py-10">
      <a className="text-sm text-[--muted] transition hover:text-white" href={withBasePath(`/category/${product.category.slug}`)}>
        До категорії {product.category.name}
      </a>

      <div className="mt-4 grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        <section className="grid gap-4">
          <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-[--surface-strong]">
            {product.coverImage ? (
              <Image
          ...
          {product.images.length > 1 ? (
            <div className="grid gap-3 sm:grid-cols-3">
              {product.images.map((image) => (
                <div key={image.id} className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-[--surface-strong]">
                  <Image
            ...
            <div className="glass-panel rounded-[2rem] p-6">
              ...
              <div className="mt-5 grid gap-3">
                {product.variants.map((variant) => (
                  <div
                    key={variant.id}
                    className="flex flex-col gap-2 rounded-[1.25rem] border border-white/10 bg-white/[0.03] px-4 py-4 md:flex-row md:items-center md:justify-between"
                  >
                    <div>
      ...
      <div className="sticky bottom-4 z-20 mt-8 lg:hidden">
        <div className="glass-panel flex items-center justify-between gap-4 rounded-full px-4 py-3">
          <div>
