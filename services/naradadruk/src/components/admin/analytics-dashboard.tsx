...
function readablePath(path: string) {
  const normalized = path.replace(/^\/naradadruk/, "") || "/";
    ...
    <section className="admin-analytics" id="analytics">
      <div className="admin-section-heading">
        <div>
          ...
          <p>
            Перегляди, кліки та переходи в Telegram за київським часом.
              ...
              >
                {report.daily.map((day, index) => (
                  <div
                    ...
                    key={day.key}
                    aria-label={`${day.label}: ${day.views} переглядів, ${day.clicks} кліків, ${day.leads} переходів у Telegram`}
                    title={`${day.label}: ${day.views} переглядів · ${day.clicks} кліків · ${day.leads} Telegram`}
                  >
