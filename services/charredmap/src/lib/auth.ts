...
function requireSecretEnv(
  ...
  const looksLikePlaceholder = Boolean(
    normalized && /^(?:change-me|replace-with|charredmap-local)/i.test(normalized),
  );
...
function getSessionSecret() {
  return requireSecretEnv(
    ...
    32,
    "charredmap-local-session-secret",
  );
}
...
export function getAdminPassword() {
  return requireSecretEnv(
    ...
    12,
    "charredmap-local-admin",
  );
}

function getAdminCookieName() {
  return isProduction() ? "__Secure-charredmap-admin" : "charredmap-admin";
}
...
export function getAdminStoriesRoute(adminPath = getAdminPath()) {
