...
import { publicationMeta, type OccupationStatus, type PublicationStatus } from "@/lib/constants";
  ...
  return (
    <div className="space-y-4">
      <form
        ...
        className="glass-panel rounded-[30px] p-5 md:p-6"
      >
        ...
        <div className="flex flex-col gap-3 border-b border-white/10 pb-5 md:flex-row md:items-end md:justify-between">
          <div className="space-y-2">
            ...
            <h1 className="font-display text-3xl text-white md:text-4xl">
              {story ? "Історія" : "Створити історію"}
            </h1>
            <p className="max-w-2xl text-sm leading-6 text-white/72">
              Мінімальна форма для модерації: текст, місто, статус і публікація.
            </p>
          </div>

          {story?.updatedAt ? (
            <p className="text-xs uppercase tracking-[0.18em] text-[--muted]">
              Оновлено {new Date(story.updatedAt).toLocaleDateString("uk-UA")}
            </p>
          ) : null}
        </div>

        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.9fr)]">
          <section className="space-y-5">
            ...
            <label className="block space-y-2">
              ...
              <textarea
                ...
                rows={14}
                maxLength={20000}
                className="min-h-[320px] w-full rounded-[26px] border border-white/10 bg-black/30 px-4 py-4 text-white outline-none transition focus:border-[--accent-orange]/60"
              />
            </label>

            <label className="block space-y-2">
              <span className="text-sm text-[--muted]">Обкладинка</span>
              <input
                name="coverImage"
                type="file"
                accept="image/*"
                className="block w-full rounded-[20px] border border-dashed border-white/12 bg-black/25 px-4 py-4 text-sm text-[--muted] file:mr-4 file:rounded-full file:border-0 file:bg-white/10 file:px-4 file:py-2 file:text-white"
              />
            </label>
          ...
          <section className="space-y-5">
            <div className="rounded-[26px] border border-white/8 bg-white/[0.03] p-4">
              <label className="block space-y-2">
                ...
                <select
                  ...
                  defaultValue={story?.publicationStatus ?? "draft"}
                ...
                >
                  {(["submitted", "draft", "published"] as PublicationStatus[]).map((status) => (
              ...
              <p className="mt-3 text-xs leading-5 text-[--muted]">
                Статус тепер йде напряму в form submit, без окремого перемикача перед збереженням.
              </p>
            </div>

            {hasSubmitterInfo ? (
              <div className="rounded-[26px] border border-[--accent-orange]/18 bg-[rgba(255,132,56,0.08)] p-4">
                <p className="text-xs uppercase tracking-[0.24em] text-[--accent-orange]">
                  Публічне подання
                </p>
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <div>
                    <p className="text-[11px] uppercase tracking-[0.18em] text-white/46">Автор</p>
                    <p className="mt-2 text-sm leading-6 text-white">
                      {story?.submitterName ?? "Не вказано"}
                    </p>
                  </div>
                  <div>
                    <p className="text-[11px] uppercase tracking-[0.18em] text-white/46">Контакт</p>
            ...
            <div className="rounded-[26px] border border-white/8 bg-white/[0.03] p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="space-y-2">
                  <p className="text-xs uppercase tracking-[0.24em] text-[--muted]">Місто</p>
                  <p className="text-sm leading-6 text-white/72">
                    Можна вибрати існуюче місто або ввести нове вручну.
                  </p>
                </div>
                {selectedLookup ? (
                  <button
                ...
                {cityLookupOpen && normalizeCitySearchValue(deferredCityName).length >= 2 ? (
                  <div className="absolute inset-x-0 top-[calc(100%+0.55rem)] z-30 overflow-hidden rounded-[24px] border border-white/10 bg-[rgba(7,9,12,0.94)] shadow-[0_30px_80px_rgba(0,0,0,0.4)] backdrop-blur-2xl">
                    {citySuggestions.length ? (
                      <div className="story-scrollbar max-h-80 overflow-y-auto p-2">
                      ...
                      <div className="px-4 py-4 text-sm leading-6 text-[--muted]">
                        Нічого не знайдено. Можна продовжити вручну.
                      </div>
                    )}
                  </div>
                ) : null}
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              <SubmitButton variant="accent">
                {story ? "Зберегти зміни" : "Створити історію"}
              </SubmitButton>
              <Link
      ...
      {story ? (
        ...
        >
          <input type="hidden" name="storyId" value={story.id} />
          <div className="space-y-2">
            <p className="text-xs uppercase tracking-[0.24em] text-[--accent-red]">Видалення</p>
            <p className="text-sm leading-6 text-white/72">
              Видаляє історію з адмінки і з публічної сторінки без можливості відкотити це з інтерфейсу.
            </p>
          </div>
          <button
            type="submit"
