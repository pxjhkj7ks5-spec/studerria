    ...
    <main className="mx-auto w-full max-w-[1400px] px-4 py-6 md:px-6 md:py-8">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[0.6fr_0.4fr]">
          ...
          <div className="glass-panel rounded-[2rem] p-6">
            ...
            <form action={updateProductAction} className="mt-6 grid gap-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="field-shell">
                  <span>Залишок без варіантів (тільки персоналу)</span>
                ...
                <div className="field-shell md:col-span-2">
                  <span>Внутрішня примітка до позиції</span>
                ...
                <div className="field-shell md:col-span-2">
                  <span>Приватне посилання на джерело / Instagram</span>
                  <input name="sourceUrl" type="url" defaultValue={product.sourceUrl} placeholder="https://www.instagram.com/..." />
                </div>
              </div>
                  ...
                  <div key={variant.id} className="rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-4">
                    <form action={saveVariantAction} className="grid gap-4">
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="field-shell">
                          <span>Залишок (тільки персоналу)</span>
                          <input name="stockQuantity" type="number" defaultValue={variant.stockQuantity} />
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-3">
                    ...
                    {staff.role === "owner" ? <form action={deleteVariantAction} className="mt-3">
                      <input type="hidden" name="productId" value={product.id} />
                      <input type="hidden" name="variantId" value={variant.id} />
        ...
        <section className="grid gap-6">
              ...
              {product.images.length > 0 ? (
                product.images.map((image) => (
                  <div key={image.id} className="rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-4">
                    ...
                    {staff.role === "owner" ? <form action={deleteProductImageAction} className="mt-3">
                      <input type="hidden" name="productId" value={product.id} />
                      <input type="hidden" name="imageId" value={image.id} />
                      <button type="submit" className="ghost-pill">
                        Видалити
          ...
          <div className="glass-panel rounded-[2rem] p-6">
            <p className="text-xs uppercase tracking-[0.28em] text-[--accent]">Складський журнал</p>
            <h2 className="mt-2 font-display text-3xl tracking-[-0.05em] text-white">Останні рухи</h2>
            <div className="mt-5 grid gap-3">
              {product.stockMovements.length ? product.stockMovements.map((movement) => (
                <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm" key={movement.id}>
                  <div className="flex justify-between gap-4"><strong>{movement.delta > 0 ? "+" : ""}{movement.delta}</strong><span className="text-[--muted]">{movement.reason}</span></div>
                  <p className="mt-1 text-xs text-[--muted]">{movement.note || "Без коментаря"} · {movement.staffUser?.displayName || "система"}</p>
                </div>
              )) : <p className="text-sm text-[--muted]">Рухів ще не було.</p>}
            </div>
          </div>

          {staff.role === "owner" ? <div className="glass-panel rounded-[2rem] p-6">
