  ...
  return (
    <main className="mx-auto w-full max-w-[1100px] px-4 py-6 md:px-6 md:py-8">
      ...
      <section className="glass-panel mt-6 rounded-[2rem] p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-2"><span className="accent-pill">{orderStatusLabel(order.status)}</span>{Object.entries(orderStatusLabels).filter(([status]) => canTransitionOrder(order.status, status as OrderStatus)).map(([status, label]) => <form action={updateOrderStatusAction} key={status}><input type="hidden" name="publicId" value={order.publicId} /><input type="hidden" name="status" value={status} /><button className="ghost-pill" type="submit">{label}</button></form>)}</div>
      ...
      {staff.role === "owner" ? <section className="glass-panel mt-6 rounded-[2rem] border border-red-400/20 p-6">
