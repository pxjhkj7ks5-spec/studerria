...
const statusLabels = { new: "Нове", accepted: "Прийнято в роботу", shipped: "Відправлено", completed: "Виконано", cancelled: "Скасовано" } as const;
    ...
    <PublicFrame telegramUrl={settings.telegramUrl}>
      <main className="order-page site-container">
        <section className="order-confirmation">
          ...
          <p>Ми отримали замовлення. Перевірити рух можна на цій захищеній сторінці.</p>
        ...
        <section className="order-summary">
          <div className="order-summary__heading"><CheckCircle aria-hidden size={22} /><h2>Історія статусу</h2></div>
          <div className="order-summary__items">
            {order.events.map((event) => <article key={`${event.eventType}-${event.createdAt.toISOString()}`}><div><strong>{event.eventType === "created" ? "Замовлення створено" : statusLabel(event.toStatus || order.status)}</strong><small>{new Intl.DateTimeFormat("uk-UA", { dateStyle: "medium", timeStyle: "short", timeZone: "Europe/Kyiv" }).format(event.createdAt)}</small></div></article>)}
          </div>
        </section>
