...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  ...
  const rawDescription = category.description.trim() || `Товари категорії «${category.name}» від YKG з оформленням замовлення на сайті та доставкою по Україні.`;
          ...
          {products.length > 0 ? (
            ...
            <div className="empty-catalog empty-catalog--light">
              <div>
                ...
                <h3>Позиції готуються до публікації.</h3>
                <p>Стежте за наступним дропом у сторінці магазину.</p>
              </div>
              <TrackedLink
