...
import { addInternalOrderComment, transitionOrderStatus } from "@/lib/order-workflow";
import {
...
function presentPublicProduct<T extends Prisma.ProductGetPayload<{ select: typeof publicProductSelect }>>(product: T) {
  const { stockQuantity, variants: rawVariants, ...safeProduct } = product;
  const images = dedupeProductImages(product.images);
  const pricing = productPricePresentation(product);
  const variants = rawVariants.map(({ stockQuantity: variantStock, ...variant }) => ({
    ...variant,
    availability: variantStock > 0 ? "in_stock" as const : "made_to_order" as const,
  ...
  return {
    ...safeProduct,
    images,
    availability: rawVariants.length > 0
      ? (rawVariants.some((variant) => variant.stockQuantity > 0) ? "in_stock" as const : "made_to_order" as const)
      : (stockQuantity > 0 ? "in_stock" as const : "made_to_order" as const),
    variants,
...
export async function getOrderByPublicId(publicId: string) {
  ...
  return prisma.order.findUnique({
    ...
    include: {
      ...
      events: {
        where: { isPublic: true },
        select: { eventType: true, fromStatus: true, toStatus: true, createdAt: true },
...
export async function updateOrderStatus(publicId: string, status: OrderStatus, actor = "admin", staffUserId?: number) {
  return transitionOrderStatus(publicId, status, { label: actor, staffUserId });
}

export async function addOrderComment(publicId: string, comment: string, actor = "admin", staffUserId?: number) {
  return addInternalOrderComment(publicId, comment, { label: actor, staffUserId });
}
...
export async function getAdminDashboardData(input?: {
  ...
  const [settings, categories, products, analyticsEvents, analyticsIpExclusions, orders, staffUsers, recentAudit] = await Promise.all([
    getSiteSettings(),
    prisma.category.findMany({
      orderBy: [{ sortOrder: "asc" }, { name: "asc" }],
      include: {
    ...
    prisma.staffUser.findMany({
      select: { id: true, username: true, displayName: true, role: true, telegramUserId: true, isActive: true, lastLoginAt: true, createdAt: true },
  ...
  return {
    ...
    telegramOperations: {
      configured: Boolean(process.env.YKG_TELEGRAM_BOT_TOKEN?.trim() && process.env.YKG_TELEGRAM_CHAT_ID?.trim()),
      topicConfigured: Boolean(process.env.YKG_TELEGRAM_TOPIC_ID?.trim()),
    },
...
async function assertProductCanPublish(productId: number, override?: { basePrice?: number | null; title?: string }) {
  ...
  if (/уточнити/iu.test(override?.title ?? product.title)) {
    throw new Error("Підтвердьте фінальну назву й приберіть позначку «уточнити».");
  }
...
export async function updateProduct(input: {
  ...
  if (input.status === ProductStatus.published) {
    await assertProductCanPublish(input.productId, { basePrice: input.basePrice, title: input.title });
  }
