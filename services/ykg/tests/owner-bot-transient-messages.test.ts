...
test("an order-comment callback cleans only its own interactive prompt and pressed button", async () => {
  const states: OwnerBotTransientStates = new Map([
    [transientStateKey("order-comment", "-1001"), { promptMessageId: 40, correctionMessageIds: [] }],
    [transientStateKey("manual", "-1001"), { promptMessageId: 90, correctionMessageIds: [] }],
  ]);
  const deleted: number[] = [];

  await advanceOwnerBotTransient(states, "order-comment", "-1001", 41, async () => ({ message_id: 42 }), async (_chatId, messageId) => {
    deleted.push(messageId);
  });

  assert.deepEqual(deleted.sort((left, right) => left - right), [40, 41]);
  assert.equal(states.get(transientStateKey("order-comment", "-1001"))?.promptMessageId, 42);
