  ...
  return <main className="auth-shell">
    <section className="auth-card auth-onboarding" aria-label="Реєстрація ShieldLine">
      ...
      {tab === "register" ? <div className="auth-form">
        ...
        <label className="auth-consent"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} /><span><i><Check size={13} /></i>Я приймаю <button type="button" onClick={(event) => { event.preventDefault(); setLegalOpen(true); }}>умови використання та обробки персональних даних</button>.</span></label>
