...
test("operational overview stays removed and placement preview stays Leaflet-native and desktop-only", async () => {
  ...
  assert.doesNotMatch(app, /MapLegend|Оперативна обстановка/);
...
test("combat results use tactical effects and desktop unit details keep readable layout", async () => {
  ...
  assert.match(map, /Дозарядити зараз ·/);
  assert.match(map, /ammoNeeded - depotStock/);
  assert.match(styles, /\.battery-action-popup__sell/);
  assert.match(styles, /\.battery-action-popup__reload/);
