...
test("service worker navigations bypass Safari's HTTP cache", async () => {
  ...
  assert.match(source, /shieldline-runtime-v11/);
  assert.match(source, /audio\/sfx\/air-raid\.mp3/);
...
test("stale offline projections cannot overwrite the migrated game state", async () => {
  ...
  assert.match(offlineStore, /PROJECTION_SCHEMA_VERSION = 4/);
  ...
  assert.match(offlineStore, /projectedMode === "campaign" && !projectedGame\?\.campaign/);
