...
Нові прив’язки створюються в `Адмінка → Розсилка розкладу`: виберіть активний курс, введіть числовий `chat ID` і, за потреби, `thread ID`. Один курс може мати кілька адрес, але одна пара `chat ID + thread ID` належить лише одному курсу.

Наведені нижче змінні залишені тільки для одноразового перенесення старої конфігурації. Після першого запуску джерелом правди є БД, а видалена в адмінці прив’язка не створюється з `.env` повторно:
...
- `CHAT_ID` — числовий ID каналу або групи. Додайте бота адміністратором каналу з правом публікувати повідомлення. Команда `/chatid` доступна dev і зареєстрованій старості та показує ID чату і, для форумів, ID теми.
...
- Час публікації спільний для всіх прив’язок і завжди рахується в `Europe/Kyiv`, з урахуванням сезонної зміни часу.
...
Для старого одноразового імпорту після зміни лише `.env`, з кореня репозиторію на сервері:

```bash
bash scripts/server-update.sh app --no-build
```

В особистому чаті з ботом `/devschedule` показує dev-користувачу список активних прив’язок. Для обраної адреси можна окремо відкрити preview або підтвердити позачергове надсилання. Ручна публікація не скасовує автоматичну о 18:00. Студенти показуються клікабельними згадками: `@username`, а без нікнейма — ім’ям. Згадки прив’язані до Telegram ID ([формат Bot API](https://core.telegram.org/bots/api#formatting-options)). Будь-який користувач може написати `/toggletag` в особистому чаті з ботом: після вимкнення у розсилці лишається username без `@` і без активної згадки; повторна команда повертає тег. Налаштування зберігається за Telegram ID і діє після прив’язки профілю. Список студентів виділено над їхніми парами, деталі занять — окремим рядком. Якщо у всіх із налаштованим розкладом немає пар, бот пише лише «😎 Завтра пар немає — чілім!» без імен, тегів і заголовка. У змішаному розкладі студенти без пар не згадуються. У заголовку — тільки день тижня та дата, без назви програми й курсу. Незавершені профілі та їхні імена не публікуються. Якщо всі профілі ще не налаштовані, публікація пропускається; dev отримує приватне пояснення. Недоступний розклад позначається окремо.
...
Сумісність: старі `STUDERRIA_TG_DAILY_SCHEDULE_*` значення імпортуються лише один раз. Подальші зміни адрес виконуються через адмінку.
